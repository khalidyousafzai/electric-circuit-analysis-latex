...
%controlled circuit
\draw (h) to [short,-o]++(0.5,0) to [short,i={$I_S{=\SI{5}{\ampere}}$}]++(0.5,0) to [short]++(0,-2) to [short]++(-0.5,0);;
\draw (l) to [short,-o]++(0.5,0);
%\draw ($(HH)!0.5!(LL)$) node{$\begin{aligned} &+ \\& v_S\\&-   \end{aligned}$};
%diamond
\draw (j)++(2.5,-1)++(1.5,0)coordinate(lowerTer) to [short,o-]++(-1.5,0)coordinate(sourceBot) to [american controlled current source]++(0,2) to [short,i={$I_0$},-o]++(1.5,0)coordinate(upperTer);
\draw (j)++(3,0) node[right]{$4 I_S$};
%text
\draw(sourceBot)++(-0.8,1)node{$\begin{aligned} &+ \\ &\SI{6}{\volt} \\ &- \end{aligned}$};
%
...
\draw($(da)!0.5!(ja)$)node[rotate=90]{\RL{برقی بوجھ}};
%
