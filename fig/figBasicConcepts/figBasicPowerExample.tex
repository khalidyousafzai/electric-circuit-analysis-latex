...
%external circuit
\draw (h) to [short]++(2,0)coordinate(rTop) to [european resistor]++(0,-2) to [short] (l);
%voltages
\draw (rTop)++(0.3,-1) node[right]{$\begin{aligned} &+\\&\SI{6}{\volt}; \, P=\SI{-24}{\watt}\\&- \end{aligned}$};
\draw ($(h)!0.5!(l)$)++(0.5,0) node{$\begin{aligned} &+\\&\SI{6}{\volt}\\&- \end{aligned}$};
%text
...
%external circuit
\draw (ha) to [short]++(2,0)coordinate(rTop) to [european resistor]++(0,-2) to [short,i={$\SI{-3}{\ampere}$}] (la);
%voltages
\draw (rTop)++(0.3,-1) node[right]{$\begin{aligned} &V_1;\, P=\SI{6}{\watt} \end{aligned}$};
\draw ($(ha)!0.5!(la)$)++(0.5,0) node[]{$\begin{aligned} &V_1 \end{aligned}$};
%text
