...
%voltages
\draw (rTop)++(0.3,-1) node[right]{$\begin{aligned} P=\SI{-15}{\watt} \end{aligned}$};
...
%external circuit
\draw (ha) to [short]++(2,0)coordinate(rTop) to [european resistor]++(0,-2) to [short,i<={$\SI{-4}{\ampere}$}] (la);
%voltages
\draw (rTop)++(0.3,-1) node[right]{$\begin{aligned}P=\SI{12}{\watt} \end{aligned}$};
\draw ($(ha)!0.5!(la)$)++(0.5,0) node[]{$\begin{aligned}  \end{aligned}$};
%text
