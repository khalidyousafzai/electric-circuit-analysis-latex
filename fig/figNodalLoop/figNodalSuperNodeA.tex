...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$\SI{8}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [american voltage source,i_={$I_m$},l={$\SI{10}{\volt}$}]++(\x,0) to [short]++(\x,0) to [american  current source,l_={$\SI{3}{\milli\ampere}$}]++(0,-\y) to [short]++(-3*\x,0);
\draw(\x,0) to [resistor,i<_={$I_1$},*-*,l={$\SI{8}{\kilo\ohm}$}]++(0,\y)node[above]{$V_1$};
\draw(2*\x,0) to [resistor,i<_={$I_2$},*-*,l={$\SI{10}{\kilo\ohm}$}]++(0,\y)node[above]{$V_2$};
