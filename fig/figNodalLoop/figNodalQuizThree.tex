...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$\SI{4}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0)node[above]{$v_1$} to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(0,-\y) to [short]++(-\x,0);
\draw(\x,\y) to [resistor,*-*,l={$\SI{6}{\kilo\ohm}$}]++(\x,0) to [american current source,l={$\SI{6}{\milli\ampere}$}]++(0,-\y) to [short,*-*]++(-\x,0);
\draw(2*\x,0) to [short]++(\x,0) to [resistor,l_={$\SI{8}{\kilo\ohm}$}]++(0,\y) to [short]++(-\x,0)node[above]{$v_2$};
