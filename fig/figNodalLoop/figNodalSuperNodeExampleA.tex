...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$\SI{20}{\volt}$}]++(0,\y)node[left]{$V_1$} to [resistor,*-,i>_={$I_1$},l={$\SI{1}{\kilo\ohm}$}]++(\x,0) node[above right]{$V_3$} to [resistor,i={$I_2$},l={$\SI{6}{\kilo\ohm}$}]++(\x,0) node[right]{$V_4$} to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(0,\y) to [short]++(-2*\x,0) to [resistor,l_={$\SI{8}{\kilo\ohm}$}]++(0,-\y);
\draw(\x,2*\y)node[above]{$V_2$} to [american voltage source,*-*,l={$\SI{16}{\volt}$}]++(0,-\y);
