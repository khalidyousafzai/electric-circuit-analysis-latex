...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$\SI{1}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [resistor,i={$I_a$},l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{8}{\kilo\ohm}$}]++(0,-\y) to [short]++(-3*\x,0);
