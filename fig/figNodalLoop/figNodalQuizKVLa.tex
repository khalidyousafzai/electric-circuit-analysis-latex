...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [short]++(2*\x,0) to [resistor,i_<={$I_a$},l={$\SI{1}{\kilo\ohm}$}]++(0,\y)node[above]{$V_2$} to [resistor,i={$I_0$},l={$\SI{2}{\kilo\ohm}$}]++(-\x,0)node[above]{$V_1$} to [short]++(-\x,0) to [resistor,l_={$\SI{4}{\kilo\ohm}$}]++(0,-\y);
\draw(\x,0) to [american controlled voltage source,*-*,l={$2000 I_a$}]++(0,\y);
\draw(2*\x,\y) to [short,*-]++(\x,0) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(0,-\y) to [short,-*]++(-\x,0);
