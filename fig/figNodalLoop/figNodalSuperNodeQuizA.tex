...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [short]++(3*\x,0) to [american voltage source,l={$\SI{8}{\volt}$}]++(0,\y) node[above]{$V_4$} to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(-\x,0) node[above]{$V_3$} to [american voltage source,l_={$\SI{3}{\volt}$}]++(-\x,0) node[above]{$V_2$} to [resistor,l_={$\SI{1}{\kilo\ohm}$}]++(-\x,0) node[above]{$V_1$} to [american voltage source,l_={$\SI{6}{\volt}$}]++(0,-\y);
\draw(\x,0) to [resistor,i<_={$I_0$},*-*,l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
