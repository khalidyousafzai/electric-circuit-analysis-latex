...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$i_A$}]++(0,\y)node[above]{$v_1$} to [short]++(\x/2,0) to [resistor,l={$R_2$}]++(\x,0)node[above]{$v_2$} to [resistor,l={$R_4$}]++(\x,0)node[right]{$v_3$} to [resistor,l={$R_5$}]++(0,-\y) to [short]++(-2.5*\x,0);
\draw(\x/2,\y) to [short,*-]++(0,\y/2) to [resistor,l={$R_3$}]++(2*\x,0) to [short,-*]++(0,-\y/2);
\draw(\x/2,0) to [resistor,*-*,l_={$R_1$}]++(0,\y);
\draw(1.5*\x,\y) to [american controlled current source,*-*,l={$\alpha v_x$}]++(0,-\y);
\draw(2*\x,\y-0.2)node[below]{$+ \, v_x \, -$};
