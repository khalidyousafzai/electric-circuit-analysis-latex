...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$\SI{8}{\volt}$}]++(0,\y)node[above]{$V_1$} to [resistor,l={$\SI{10}{\kilo\ohm}$}]++(\x,0)node[above]{$V_2$} to [resistor,l={$\SI{8}{\kilo\ohm}$}]++(\x,0)node[above]{$V_3$} to [resistor,l_={$\SI{3}{\kilo\ohm}$}]++(0,-\y) to [short]++(-2*\x,0);
\draw(\x,0) node[ground]{};
\draw(\x,0) to [resistor,*-*,i<_={$I_0$},l={$\SI{20}{\kilo\ohm}$}]++(0,\y);
\draw(2*\x,0) to [short,*-]++(\x/2,0) to [american voltage source,l_={$\SI{6}{\volt}$}]++(0,\y) to [short,-*]++(-\x/2,0);
\end{tikzpicture}%
\end{urdufont}
\end{document}
