...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$i_A$}]++(0,\y)node[above]{$v_1$} to [short]++(\x/2,0) to [resistor,i>_={$i_2$},l={$R_d$}]++(\x,0)node[above]{$v_2$}  to [resistor,l={$R_e$}]++(\x,0)  to [short]++(\x/2,0)node[above]{$v_3$} to [american current source,l={$i_B$}]++(0,-\y) to [short]++(-3*\x,0);
\draw(\x/2,0) to [resistor,i^<={$i_1$},*-*,l_={$R_a$}]++(0,\y);
\draw(1.5*\x,0) to [resistor,*-*,l={$R_b$}]++(0,\y);
