...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0)node[left]{$v_1$} to [short,*-]++(0,\y/2) to [american current source,l={$\SI{4}{\milli\ampere}$}]++(2*\x,0) to [short]++(0,-\y/2)node[right]{$v_3$} to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(-\x,0)node[above]{$v_2$} to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(-\x,0);
\draw(\x,0) to [american current source,*-*,l={$\SI{2}{\milli\ampere}$}]++(0,-\y) to [short]++(-\x,0) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\draw(2*\x,0) to [resistor,*-,l={$\SI{4}{\kilo\ohm}$}]++(0,-\y) to [short]++(-\x,0);
