...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={$\SI{4}{\milli\ampere}$}]++(0,\y) to [short]++(2*\x,0)node[above]{$V_a$} to [american controlled current source,l={$\frac{V_a}{3000}$}]++(\x,0)node[above]{$V_0$} to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(0,-\y) to [short]++(-3*\x,0);
