...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [resistor,i<={$i_1$},l={$R_1$}]++(0,\y) to [short]++(\x,0) to [resistor,i>_={$i_2$},l={$R_2$}]++(\x,0) to [short]++(\x,0) to [american current source,l={$i_B$}]++(0,-\y) to [short]++(-3*\x,0);
\draw(\x,0) to [american current source,*-*,l={$i_A$}]++(0,\y)node[above]{$v_1$};
\draw(2*\x,0) to [resistor,i<={$i_3$},*-*,l={$R_3$}]++(0,\y)node[above]{$v_2$};
