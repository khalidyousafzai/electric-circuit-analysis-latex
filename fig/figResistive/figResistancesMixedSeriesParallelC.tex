...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0)node[left]{$B$} to [resistor,o-,l_={$\frac{1}{3}\, \si{\kilo\ohm}$}]++(\x,0)node[below]{$D$} to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(\x,0)node[below]{$F$} to [short]++(\x,0) to [resistor,l_={$\SI{2.4}{\kilo\ohm}$}]++(0,\y)node[above]{$G$};
