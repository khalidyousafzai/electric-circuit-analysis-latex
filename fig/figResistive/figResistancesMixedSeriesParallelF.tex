...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0)node[left]{$B$} to [resistor,o-,l_={$\frac{1}{3}\, \si{\kilo\ohm}$}]++(\x,0)node[below]{$D$} to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(\x,0)node[below]{$F$}to [short]++(0,\y);
\draw(0,\y)node[left]{$A$} to [resistor,o-,l={$\frac{1}{3}\,\si{\kilo\ohm}$}]++(\x,0)node[above]{$C$} to [resistor,l={$\SI{3}{\kilo\ohm}$}]++(\x,0);
