...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$\SI{22}{\volt}$}]++(0,\y) to [resistor,l={$\SI{10}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{20}{\kilo\ohm}$}]++(\x,0) to [resistor,l_={$\SI{10}{\kilo\ohm}$}]++(0,-\y) to [short]++(-2*\x,0);
\draw(\x,0) to [resistor,l={$\SI{15}{\kilo\ohm}$},*-*]++(0,\y);
