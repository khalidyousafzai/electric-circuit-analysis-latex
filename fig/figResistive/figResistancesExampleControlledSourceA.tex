...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$\SI{20}{\volt}$}]++(0,\y) to [resistor,i>^={$i$},l_={$\SI{1.2}{\kilo\ohm}$}]++(\x,0) to [american controlled voltage source,l={${V_0=3V_1}$}]++(\x,0) to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(0,-\y) to (0,0);
\draw(\x/2,\y+0.2)node[above]{$+ \, V_1 \, -$};
\draw(2*\x+0.2,\y/2)node[right]{$\begin{aligned} &+ \\ &V_2\\ &- \end{aligned}$};
\end{tikzpicture}%
\end{urdufont}
\end{document}
