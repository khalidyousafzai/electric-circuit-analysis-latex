...
\begin{tikzpicture}[]
%\boxLeft[p]{0,0} 
\draw (0,0) to [american voltage source]++(0,\y) to [short,i={$I$}]++(\x,0) to [resistor,i={$\SI{3}{\milli\ampere}$}] ++(0,-\y);
\draw (\x,\y) to [short,*-]++(\x,0) to [resistor,i={$\SI{5}{\milli\ampere}$}]++(0,-\y) to [short,-*] ++(-\x,0) to [short,*-]++(-\x,0);
%
\end{tikzpicture}%
\end{urdufont}
\end{document}
