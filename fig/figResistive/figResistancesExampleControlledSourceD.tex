...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$\SI{16}{\volt}$}]++(0,\y) to [resistor,i>^={$I_1$},l={$\SI{5}{\kilo\ohm}$}]++(\x,0) to [american controlled voltage source,l={${V_m=3000 I_1}$}]++(\x,0) to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(0,-\y) to [short]++(-2*\x,0);
