...
\begin{tikzpicture}[]
%
\draw(0,0) to [american current source,l={$\SI{8}{\milli\ampere}$}]++(0,\y) to [short] ++(\x,0) to [resistor,i={$I_1$}]++(0,-\y) to [short]++(-\x,0);
\draw(0,0) to [short,*-] ++(-\x,0) to [resistor,i<_={$\SI{3}{\milli\ampere}$}]++(0,\y) to [short,-*]++(\x,0);
\end{tikzpicture}%
\end{urdufont}
\end{document}
