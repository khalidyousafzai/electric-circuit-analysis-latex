...
\draw(0,0) to [american voltage source,l={$v(t)$}]++(0,2*\y) to [short,i={$i(t)$}]++(\x,0) to [resistor,l_={$R_1$}]++(0,-\y) to [resistor,l_={$R_2$}]++(0,-\y) to [short]++(-\x,0);
\draw (\x+0.2,\y+\y/2) node[right]{$\begin{aligned} &+ \\ & v_{R1}=i(t)R_1 \\ &-\end{aligned}$};
\draw (\x+0.2,\y/2) node[right]{$\begin{aligned} &+ \\ & v_{R2}=i(t)R_2 \\ &-\end{aligned}$};
\end{tikzpicture}%
\end{urdufont}
\end{document}
