...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$V_m$}]++(0,\y) to [resistor,l={$\SI{10}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{20}{\kilo\ohm}$}]++(\x,0) to [resistor,l_={$\SI{10}{\kilo\ohm}$}]++(0,-\y) to [short]++(-2*\x,0);
\draw(\x,0) to [resistor,i<_={$\SI{2}{\milli\ampere}$},l={$\SI{15}{\kilo\ohm}$},*-*]++(0,\y);
\end{tikzpicture}%
\end{urdufont}
\end{document}
