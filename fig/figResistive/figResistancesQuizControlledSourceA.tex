...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american voltage source,l={$v_s$}]++(0,\y) to [resistor,l={$R_S$}]++(\x,0) to [resistor,l={$r_{be}$},i>^={$i_b$}]++(0,-\y) to [short]++(-\x,0);
\draw(\x,0) to [short,*-]++(2.5*\x,0) to [resistor,l={$R_C$}] ++(0,\y) to [short]++(-1.5*\x,0) to [american controlled current source,-*,l={${I_0=\beta i_b}$}] ++(0,-\y) ;
\draw(3.5*\x,0) to [short,*-]++(\x,0)  to [resistor,l={$R_L$},i_<={$i_L$}]++(0,\y) to [short,-*]++(-\x,0);
\end{tikzpicture}%
\end{urdufont}
\end{document}
