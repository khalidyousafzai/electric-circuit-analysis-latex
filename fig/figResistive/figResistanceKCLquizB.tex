...
\begin{tikzpicture}[]
%
\draw(0,0) to [american voltage source,i={$\SI{17}{\milli\ampere}$}]++(0,\y) to [resistor,i={$I_2$}] ++(\x,0) to [resistor,i={$\SI{7}{\milli\ampere}$}]++(0,-\y) to [short]++(-\x,0);
\draw(\x,\y) to [short,*-]++(\x,0) to [resistor,i={$\SI{4}{\milli\ampere}$}]++(0,-\y) to [short]++(-\x,0);
\draw(0,0) to [short,*-] ++(-\x,0) to [resistor,i<_={$I_3$}]++(0,\y) to [short,-*]++(\x,0);
\end{tikzpicture}%
\end{urdufont}
\end{document}
