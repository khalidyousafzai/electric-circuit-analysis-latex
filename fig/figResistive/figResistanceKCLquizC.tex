...
\begin{tikzpicture}[]
%
\draw(0,0) to [american current source,l={$\SI{18}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [resistor,i={$i_1$}] ++(0,-\y) to [short]++(-\x,0);
\draw(\x,\y) to [short,*-]++(\x,0) to [american controlled current source,l={$5 i_1$}]++(0,-\y) to [short,-*]++(-\x,0);
\end{tikzpicture}%
\end{urdufont}
\end{document}
