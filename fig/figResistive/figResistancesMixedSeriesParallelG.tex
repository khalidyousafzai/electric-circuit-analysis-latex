...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0)node[left]{$B$} to [resistor,o-,l_={$\frac{1}{3}\, \si{\kilo\ohm}$}]++(\x,0)node[below]{$D$} to [short]++(\x,0)node[below]{$F$}to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(0,\y);
\draw(0,\y)node[left]{$A$} to [resistor,o-,l={$\frac{1}{3}\,\si{\kilo\ohm}$}]++(\x,0)node[above]{$C$} to [short]++(\x,0);
