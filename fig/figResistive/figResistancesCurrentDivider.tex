...
%\boxLeft[p]{0,0} 
%grid
\draw(0,0) to [american current source,l={${i(t)}$}]++(0,\y) to [short]++(\x,0)node[above]{$b$} to [resistor,i>_={$i_1(t)$},l={$R_1$}]++(0,-\y)node[below]{$a$}--(0,0); 
\draw(\x,0) to [short,*-]++(\x,0) to [resistor,i<={$i_2(t)$},l_={$R_2$}]++(0,\y)to [short,-*]++(-\x,0);
%text
