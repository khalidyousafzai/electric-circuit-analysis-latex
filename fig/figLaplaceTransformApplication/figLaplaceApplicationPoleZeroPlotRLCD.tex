...
\usepackage{bm}	
\usepackage{commath}	
\usepackage{siunitx}								%differentiation symbols
\usepackage{pgfplots}
\usepackage{tikz}
...
\begin{tikzpicture}
\begin{axis}[zlabel={$\abs{\bH(s)}$},xlabel={$\delta$},ylabel={${\omega \,\si{\radian\per\second}}$},zlabel style={rotate=-90},colormap={bw}{gray(0 cm)=(0);gray(1 cm)=(1)},zmax=3,zmin=0,view={90}{0}]
...
\addplot3[y domain=0:4, black,ultra thick,samples y=40](0,y,{4*sqrt(y^2)/sqrt((-y^2+8)^2+(4*y)^2)+0.001});
\end{axis}
