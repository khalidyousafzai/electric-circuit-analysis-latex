...
%====================
\ابتدا{مثال}
بدلتی رو \عددی{i(t)=I_0\cos (\omega t+\phi)} کی موثر قیمت \عددی{\Irms} دریافت کریں۔
...
\end{align}
لکھا جا سکتا ہے۔چونکہ \عددی{\sin(\omega t +\phi)=\cos(\omega t +\phi-90^{\circ})} لکھا جا سکتا ہے لہٰذا سائن موج کی موثر قیمت بھی درج بالا ہو گی۔اسی طرح \عددی{V_0} حیطے کے سائن نما دباو کی موثر قیمت درج ذیل ہو گی۔
...
اب تک ہم دباو یا رو کا حیطہ لیتے ہوئے ان کے دوری سمتیات لکھتے رہے ہیں مثلاً  \عددی{\hat{V}=V_0\phase{\phi^{\circ}}\,\si{\volt}} دوری سمتیہ \عددی{V_0} حیطے اور \عددی{\phi} زاویہ ہٹاو کے کوسائن دباو کو ظاہر کرتا ہے۔ہم دوری سمتیات کو موثر قیمت کی صورت میں بھی لکھ سکتے ہیں۔یوں
 \عددی{\hat{V}_{\textup{rms}}=230\phase{\phi}\,\si{\volt}} میں \عددی{\SI{230}{\volt}} دباو کے موثر قیمت کو ظاہر کرتی ہے  جبکہ
 \عددی{\hat{V}=325\phase{\phi}\,\si{\volt}} میں \عددی{\SI{325}{\volt}} دباو کا حیطہ ہے۔تسلی کر لیں کہ یہ دونوں دوری سمتیات ایک ہی دباو کو ظاہر کرتی ہیں۔
...
\فرہنگ{power!real} کہا جاتا ہے۔ظاہری طاقت کو وولٹ-ایمپیئر \عددی{\si{\volt\ampere}} میں ناپا جاتا ہے جبکہ حقیقی طاقت کو واٹ \عددی{\si{\watt}} میں ناپا جاتا ہے۔یاد رہے کہ \عددی{\cos(\phi_v-\phi_i)} بے بعد مقدار ہے لہٰذا حقیقی طاقت کا بعد بھی حقیقت میں وولٹ-ایمپیئر \عددی{\si{\volt\ampere}} ہی ہے جسے واٹ \عددی{\si{\watt}} کا خصوصی نام دیا گیا ہے۔حقیقی طاقت اور ظاہری طاقت میں فرق ظاہر کرنے اور انہیں پہچانے کی خاطر ان کی اکایوں کو علیحدہ عیلحدہ نام دیے گئے ہیں۔
...
\begin{align}
\text{\RL{جزو طاقت}}=\pf=\frac{P}{\Vrms\Irms}=\cos(\phi_v-\phi_i)
\end{align}
جہاں 
\begin{align}
\cos(\phi_v-\phi_i)=\cos \phi_{z}
\end{align}
کے برابر ہے۔زاویہ \عددی{\phi_v-\phi_i} درحقیقت بوجھ کے رکاوٹ کا زاویہ ہٹاو \عددی{\phi_{z}} ہے اور اسے \اصطلاح{زاویہ جزو طاقت}\فرہنگ{زاویہ جزو طاقت}\حاشیہب{power factor angle}\فرہنگ{power factor angle} کہا جاتا ہے۔چونکہ \عددی{\cos(\phi_v-\phi_i)=\cos(\phi_i-\phi_v)} کے برابر ہے لہٰذا جزو طاقت کو  امالی بوجھ کی صورت میں \اصطلاح{امالی جزو طاقت}\فرہنگ{جزو طاقت!امالی}\فرہنگ{امالی!جزو طاقت}\حاشیہب{inductive power factor}\فرہنگ{power factor!inductive}
...
حل:شکل \حوالہ{شکل_طاقت_جزو_طاقت_کے_اثرات} میں صورت حال دکھائی گئی ہے۔مساوات \حوالہ{مساوات_طاقت_عمومی_پ} سے رو حاصل کرتے ہیں جہاں \عددی{\cos(\phi_v-\phi_i)} جزو طاقت ہے۔
\begin{align*}
...
\انتہا{مثال}
%====================
آئیں مساوات \حوالہ{مساوات_طاقت_مخلوط_طاقت_پ} اور مساوات \حوالہ{مساوات_طاقت_مخلوط_طاقت_ت} پر مزاحمت، امالہ اور برق گیر کے نقطہ نظر سے مزید غور کریں۔مزاحمت کا \عددی{\phi_v-\phi_i=0^{\circ}} ہے لہٰذا اس کے \عددی{\cos(\phi_v-\phi_i)=1} اور \عددی{\sin(\phi_v-\phi_i)=0} ہیں۔یوں مزاحمت حقیقی طاقت جذب \عددی{(P>0)} کرتا ہے جبکہ یہ متعاملی طاقت کو جذب نہیں کرتا لہٰذا \عددی{Q=0} ہے۔امالہ کا \عددی{\phi_v-\phi_i=90^{\circ}}  لہٰذا
\begin{align}
P&=\Vrms \Irms \cos 90^{\circ}=0\\
Q&=\Vrms \Irms \sin 90^{\circ}>0
\end{align}
ہیں۔امالہ گیر متعاملی طاقت کو جذب کرتا ہے جبکہ یہ حقیقی طاقت کو جذب نہیں کرتا۔برق گیر کا \عددی{\phi_v-\phi_i=-90^{\circ}} لہٰذا
\begin{align}
P&=\Vrms \Irms \cos (-90^{\circ})=0\\
Q&=\Vrms \Irms \sin (-90^{\circ})<0
\end{align}
...
جس کو شکل-ب کے \اصطلاح{طاقتی تکون}\فرہنگ{طاقتی تکون}\فرہنگ{تکون!طاقت}\حاشیہب{power triangle}\فرہنگ{power triangle} سے بھی حاصل کیا جا سکتا ہے۔
 شکل-ب امالی بوجھ کے لئے دکھایا گیا ہے جہاں \عددی{\phi_v-\phi_i>0} ہے۔چونکہ زاویہ افقی محور سے گھڑی کی گردش کے الٹ جانب گھومتے ہوئے ناپا جاتا ہے لہٰذا مثبت زاویہ حقیقی محور سے اوپر کو ہو گا۔یوں امالی بوجھ کا \عددی{Q} مثبت ہے۔برق گیر بوجھ کی صورت میں \عددی{\phi_v-\phi_i<0} ہو گا لہٰذا \عددی{\bS} کا خط حقیقی محور سے نیچے کو ہو گا لہٰذا \عددی{Q} کی قیمت منفی ہو گی۔مزاحمتی بوجھ کی صورت میں \عددی{\phi_v-\phi_i=0} ہو گا لہٰذا \عددی{\bS} کا خط عین حقیقی محور پر ہو گا لہٰذا مخلوط طاقت اور حقیقی طاقت برابر ہوں گے جبکہ متعاملی طاقت صفر ہو گا۔
...
\draw[latex-](iTip)--($(vTip)!(iTip)!(O)$)coordinate(iDrop)node[pos=0.5,right]{$\bIrms \sin(\phi_v-\phi_i)$};
\draw[-latex](O)--(iDrop)node[pos=0.55,above,sloped]{$\bIrms \cos(\phi_v-\phi_i)$};
\draw[-stealth] ([shift={(0:1)}]0,0) arc (0:\angV:1);
\draw(3/4*\angV:1.3)node[]{$\phi_v$};
\draw[-stealth] ([shift={(0:1.3)}]0,0) arc (0:\angI:1.3);
\draw(1/2*\angI:1.6)node[]{$\phi_i$};
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}%
\begin{subfigure}{0.5\textwidth}
...
\draw(1/2*\angS:1.3)node[right]{$\phi_v-\phi_i$};
...
\end{align*}
ہے جبکہ بوجھ پر \عددی{\phi_v-\phi_i=\cos^{-1}(0.9)=25.84^{\circ}} ہے لہٰذا بوجھ پر
\begin{align*}
...
\draw(3/4*33:0.8)node[right]{$\phi_v-\phi_i$};
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}%
...
\draw(3/4*18:1.3)node[right]{$\phi_{\text{بہتر}}$};
\end{tikzpicture}
\caption*{(پ)}
\end{subfigure}%
...
\text{\RL{مشین}}&=1000 \times 1 \times 1 \times 3600=\SI{3600000}{\joule}\\
...
\end{align*}
ان کا مجموعہ \عددی{\SI{50.526}{\mega\joule}} ہے جس سے مہینے میں خرچ ہونے والی توانائی درج ذیل حاصل ہوتی ہے۔
\begin{align*}
\SI{50.526}{\mega \joule} \times 30 \approx \SI{1.52}{\giga\joule}
\end{align*}
...
مثال \حوالہ{مثال_طاقت_چھوٹا_گھرانا_الف} میں آپ نے دیکھا کہ چھوٹے گھرانے کی توانائی کا تخمینہ لگاتے ہوئے بھی ہمیں بڑے بڑے اعداد کا سامنہ پڑا جس سے ثابت ہوتا ہے کہ توانائی کی اکائی جاول \عددی{\si{\joule}} انتہائی چھوٹی اکائی ہے۔مثال \حوالہ{مثال_طاقت_چھوٹا_گھرانا_ب} میں ایک کلو واٹ مشین کی ایک گھنٹے کی  توانائی حاصل کی گئی۔بل تیار کرتے وقت اسی مقدار کو اکائی تصور کیا جاتا ہے۔یوں گھریلو اور صنعتی صارفین کے توانائی کا خرچ \اصطلاح{کلو واٹ گھنٹوں}\فرہنگ{کلو واٹ گھنٹہ}\حاشیہب{kilowatt-hour, kWh}\فرہنگ{kWh}\فرہنگ{kilowatt-hour} \عددی{\si{\kilo\watt\hour}} میں ناپا جاتا ہے۔توانائی کا نرخ اسی اکائی کے لئے بیان کیا جاتا ہے۔
\begin{align}
\SI{1}{\kilo\watt\hour}=\SI{3.6}{\mega\joule} \quad \text{\RL{توانائی کی اکائی}}
\end{align}
مثال \حوالہ{مثال_طاقت_چھوٹا_گھرانا_الف} کے صارف کے ماہوار بل میں درج ذیل درج ہو گا۔
\begin{align*}
\frac{\SI{1.52}{\giga\joule}}{\SI{3.6}{\mega\joule}}=\SI{422.2}{\kilo\watt\hour}
\end{align*}
اب فرض کریں کہ توانائی کی قیمت دس روپے فی کلو واٹ گھنٹہ ہے تب اس گھرانے کا ماہوار بل \عددی{4222} روپے ہو گا۔ 
