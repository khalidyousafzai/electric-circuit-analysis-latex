...
\begin{align*}
I^2_{rms}=\frac{1}{T}\int_{0}^{T}I_0^2 \cos^2(\omega t +\phi) \dif t
\end{align*} 
یہاں \عددی{\cos^2 \alpha=\tfrac{1+\cos 2\alpha}{2}} کا استعمال کرتے ہوئے درج ذیل لکھا جا سکتا ہے
\begin{align*}
I^2_{rms}=\frac{I_0^2 }{T}\int_{0}^{T}\frac{1}{2} \dif t+\frac{I_0^2 }{T}\int_{0}^{T} \frac{\cos 2(\omega t +\phi)}{2} \dif t
\end{align*}
جس میں دوسرا تکمل صفر کے برابر ہے۔پہلا تکمل حل کرتے ہوئے
\begin{align*}
I^2_{rms}=\left. \frac{I_0^2 }{T}\frac{1}{2} t\right |_{0}^{T}
\end{align*}
یعنی
...
اب تک ہم دباو یا رو کا حیطہ لیتے ہوئے ان کے دوری سمتیات لکھتے رہے ہیں مثلاً  \عددی{\hat{V}=V_0\phase{\phi^{\circ}}\,\si{\volt}} دوری سمتیہ \عددی{V_0} حیطے اور \عددی{\phi} زاویہ ہٹاو کے کوسائن دباو کو ظاہر کرتا ہے۔ہم دوری سمتیات کو موثر قیمت کی صورت میں بھی لکھ سکتے ہیں۔یوں
 \عددی{\hat{V}_{rms}=230\phase{\phi}\,\si{\volt}} میں \عددی{\SI{230}{\volt}} دباو کے موثر قیمت کو ظاہر کرتی ہے  جبکہ
...
دباو یا رو کی قیمتیں مختلف انداز میں بیان کی جا سکتی  ہیں۔مثلاً مساوات \حوالہ{مساوات_طاقت_پاکستانی_گھریلو_دباو} میں دباو کی چوٹی \عددی{V_p} یا مثبت اور منفی چوٹیوں کے درمیان قیمت \عددی{V_{pp}} اور یا پھر دباو کی موثر قیمت \عددی{V_{rms}} بیان کی جا سکتی ہے۔یوں درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
V_p&=\SI{325}{\volt}\\
V_{pp}&=\SI{650}{\volt}\\
V_{rms}&=\SI{230}{\volt}
\end{align*}
بدلتی رو مشینوں کی دباو اور رو کی عموماً موثر قیمتیں بیان کی جاتی ہیں۔یوں \عددی{\SI{230}{\volt}} پر چلنے والا گھریلو پنکھا درحقیقت \عددی{\SI{230}{\volt}} کے موثر دباو پر چلے گا۔اس کتاب میں موثر قیمتیں استعمال کرتے ہوئے دباو اور رو کے ساتھ موثر (یا \عددی{rms}) لکھا جائے گا۔
...
\end{align*}
چونکہ \عددی{\tfrac{V_0}{\sqrt{2}}} اور \عددی{\tfrac{I_0}{\sqrt{2}}} بالترتیب موثر دباو \عددی{V_{\text{rms}}} اور موثر رو \عددی{I_{\text{rms}}} ہیں لہٰذا درج بالا مساوات کو درج ذیل لکھا جا سکتا ہے۔
...
\begin{align}
P&=\frac{I_0^2 R}{2}=I^2_{rms} R\\
P&=\frac{V_0^2}{2R}=\frac{V^2_{rms}}{R}
\end{align}
...
شکل \حوالہ{شکل_طاقت_موثر_قیمتیں_الف} میں دیے دباو کی موثر قیمت دریافت کریں۔اگر \عددی{D=\SI{50}{\percent}} اور \عددی{V_p=\SI{60}{\volt}} ہوں تب یہ دباو \عددی{\SI{200}{\ohm}} مزاحمت کو کتنی طاقت مہیا کر سکتا ہے اور مزاحمت کی موثر رو کیا ہو گی۔
\begin{figure}
\centering
\begin{tikzpicture}
\pgfmathsetmacro{\T}{2};
...
&=\sqrt{\frac{1}{T}\left[\int_{0}^{t_1} V_p^2 \dif t +\int_{t_1}^{T} 0^2 \dif t\right]}\\
&=V_p\sqrt{\frac{t_1}{T}}\\
&=V_p\sqrt{D}
\end{align*}
حاصل ہوتا ہے۔فعال عرصے کی قیمت \عددی{0<D<1} ممکن ہے جس سے \عددی{V_{rms}} کی قیمت \عددی{\SI{0}{\volt}} تا \عددی{V_p \,\si{\volt}} ممکن ہے۔

دی گئی معلومات کے مطابق موثر دباو درج ذیل ہے
\begin{align*}
V_{rms}=60\sqrt{0.5}=\SI{42.4264}{\volt}
\end{align*}
جسے \عددی{\SI{200}{\ohm}} کے  متوازی لاگو کرنے سے مزاحمت کو درج ذیل طاقت مہیا ہو گی۔
\begin{align*}
P=\frac{V^2_{rms}}{R}=\frac{42.4264^2}{200}=\SI{9}{\watt}
\end{align*}
مزاحمت کی موثر رو درج ذیل ہو گی۔
\begin{align*}
I_{rms}=\frac{V_{rms}}{R}=\frac{42.4264}{200}=\SI{0.212}{\ampere}
\end{align*}
\انتہا{مثال}
%===============
\ابتدا{مثال}\شناخت{مثال_طاقت_موثر_قیمتیں_ب}
شکل \حوالہ{شکل_طاقت_موثر_قیمتیں_ب} میں \عددی{D} کی قیمت \عددی{\SI{30}{\percent}}، \عددی{\SI{50}{\percent}} اور \عددی{\SI{70}{\percent}} کی صورت میں  دباو کی موثر قیمت اور اوسط قیمت دریافت کریں جہاں \عددی{V_p=\SI{10}{\volt}} ہے۔
\begin{figure}
\centering
\begin{tikzpicture}
\pgfmathsetmacro{\T}{2};
...
\begin{align*}
V_{\text{موثر}}&=\sqrt{\frac{1}{T}\int_{0}^{T} v^2(t) \dif t}\\
&=\sqrt{\frac{1}{T}\left[\int_{0}^{t_1} V_p^2 \dif t +\int_{t_1}^{T} (-V_p)^2 \dif t\right]}\\
&=V_p\sqrt{\frac{t_1}{T}+\frac{T-t_1}{T}}\\
&=V_p
\end{align*}
...
&=\frac{1}{T} \left[\int_{0}^{t_1}V_p \dif t+\int_{t_1}^{T} (-V_p) \dif t\right]\\
&=V_p \left(\frac{2t_1-T}{T}\right)\\
&=V_p(2D-1)
\end{align*}
...
\begin{align*}
v_{rms}&=\sqrt{\frac{1}{T} \int_{0}^{T} v^2(t) \dif t}\\
...
P&=V_{rms} I_{rms} \cos(\phi_v-\phi_i)
\end{align}
اس مساوات میں \عددی{V_{rms}I_{rms}} کے حاصل ضرب کو \اصطلاح{ظاہری طاقت}\فرہنگ{ظاہری طاقت}\فرہنگ{طاقت!ظاہری}\حاشیہب{apparent power}
\فرہنگ{power!apparent} کہا جاتا ہے جبکہ \عددی{P} کو \اصطلاح{حقیقی طاقت}\فرہنگ{حقیقی طاقت}\فرہنگ{طاقت!حقیقی}\حاشیہب{real power}
\فرہنگ{power!real} کہا جاتا ہے۔ظاہری طاقت کو وولٹ-ایمپیئر \عددی{\si{\volt\ampere}} میں ناپا جاتا ہے جبکہ حقیقی طاقت کو واٹ \عددی{\si{\watt}} میں ناپا جاتا ہے۔ایسا کرنے سے ان میں فرق کرنا ممکن ہوتا ہے۔
...
\begin{align}
\text{\RL{جزو طاقت}}=\pf=\frac{P}{V_{rms}I_{rms}}=\cos(\phi_v-\phi_i)
\end{align}
جہاں 
\begin{align}
