...
برقی میدان میں \عددی{y(t)} وقت کے ساتھ بدلتے دباو یا وقت کے ساتھ بدلتا رو کو ظاہر کر سکتی ہے۔مساوات \حوالہ{مساوات_بدلتا_سائن_نما_تفاعل_ب} میں دیے تفاعل، جسے شکل \حوالہ{شکل_بدلتی_رو_سائن_تفاعل}-ب میں دکھایا گیا ہے،  کا آزاد متغیرہ وقت \عددی{t} ہے۔آپ دیکھ سکتے ہیں کہ یہ تفاعل ہر \عددی{T} سیکنڈ کے بعد اپنے آپ کو دہراتا ہے۔ اس حقیقت کو ریاضی میں درج ذیل لکھا جاتا ہے۔
\begin{align}
y(t+T)=y( t)
\end{align}
...
گزشتہ باب میں دور پر مستقل جبری تفاعل مسلط  کرتے ہوئے، دور کا جبری ردعمل بھی مستقل قیمت کا حاصل ہوا۔تفرقی مساوات کا جبری ردعمل، مسلط جبری تفاعل اور اس کے تمام بلند رتبی تفرق کا مجموعہ ہوتا ہے۔یوں  دور پر جبری دباو \عددی{v(t)=\sin \omega t} مسلط کرنے سے رو کا جبری ردعمل \عددی{i(t)=c_1\sin\omega t+c_2\cos\omega t} متوقع ہو گا۔پس جبری ردعمل کے مستقل \عددی{c_1} اور \عددی{c_2} معلوم کرنا باقی ہے۔
%===============
...
\end{align}
چونکہ دباو اور رو کی شرح کو اوہم \عددی{\si{\ohm}} میں ناپتے ہیں لہٰذا رکاوٹ کی اکائی بھی اوہم ہے۔یوں بدلتا رو دور کی رکاوٹ یک سمت رو دور کی مزاحمت کی مانند ہے۔رکاوٹ کو مستطیل طرز میں بھی لکھا جا سکتا ہے
\begin{align}
...
%=============
\ابتدا{مشق}
صفحہ \حوالہصفحہ{شکل_مزاحمتی_متعدد_مزاحمت_تقسیم_دباو} پر شکل \حوالہ{شکل_مزاحمتی_متعدد_مزاحمت_تقسیم_دباو} میں سلسلہ وار مزاحمت جڑے دکھائے گئے ہیں۔مزاحمتوں کی جگہ رکاوٹ نصب کرتے ہوئے، مخلوط دباو اور مخلوط رو کے استعمال سے مساوی رکاوٹ کی مساوات حاصل کریں۔ اسی طرح متعدد رکاوٹوں کو متوازی جوڑتے ہوئے ان کا مساوی رکاوٹ حاصل کریں۔
...
&=10.816\phase{-74.35^{\circ}}\,\si{\volt}
\end{align*}
اس کے علاوہ \عددی{\SI{1}{\ohm}}  مزاحمت میں \عددی{\hat{I}_3} گزرتی ہے۔یوں \عددی{\hat{V}_1} سے اس مزاحمت کا دباو منفی کرنے سے بھی برق گیر کا دباو حاصل کیا جا سکتا ہے یعنی
\begin{align*}
...
حل:مسئلہ خطی میل میں تمام منبع کے انفرادی اثرات کا مجموعہ لیا جاتا ہے۔شکل \حوالہ{شکل_بدلتا_تراکیب_ب} میں منبع دباو کو قصر دور کرتے ہوئے  شکل \حوالہ{شکل_بدلتا_تراکیب_پ} حاصل ہوتا ہے۔منبع رو کے متوازی کُل رکاوٹ \عددی{ \bZ_1} حاصل کرتے ہیں۔شکل کو دیکھتے ہوئے درج ذیل لکھا جا سکتا ہے
\begin{align*}
...
آئیں اب منبع دباو سے پیدا رو حاصل کریں۔ایسا کرنے کی خاطر منبع رو کو کھلا دور کرتے ہوئے شکل \حوالہ{شکل_بدلتا_تراکیب_ت} حاصل کرتے ہیں۔اس شکل میں \عددی{\hat{I}} جانتے ہوئے تقسیم رو کے کلیے سے \عددی{\hat{I}''_0} حاصل کیا جا سکتا ہے۔آئیں پہلے \عددی{\hat{I}} حاصل کریں۔منبع رو کے ساتھ کُل درج ذیل رکاوٹ جڑی ہے۔
\begin{align*}
...
حل:منبع رو کے متوازی رکاوٹ \عددی{2-j4} جڑی ہے۔ان کو نارٹن مساوی دور تصور کرتے ہوئے ان کی جگہ  تھونن مساوی  دور نصب کرتے ہوئے شکل-ب ملتا ہے جہاں درج ذیل متبادل منبع دباو نصب کیا گیا ہے۔
\begin{align*}
...
حل:تھونن دباو دریافت کرنے کی خاطر بوجھ کو کھلا دور کرنے سے شکل-ب حاصل ہوتا ہے جہاں متوازی جڑے منبع رو اور رکاوٹ \عددی{2-j2 \,\si{\ohm}} کا متبادل تھونن دور نصب کیا گیا ہے۔دور میں رو \عددی{\hat{I}} تصور کرتے ہوئے درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
...
\end{align*}
 تھونن رکاوٹ حاصل کرنے کی خاطر شکل \حوالہ{شکل_بدلتا_تراکیب_ث} میں منبع رو کو کھلے دور اور منبع دباو کو قصر دور کرتے ہوئے شکل-پ حاصل ہوتا ہے جہاں سے درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
...
حل:یہاں منبع دباو کی تعدد مختلف ہے لہٰذا دوری سمتیات استعمال کرتے ہوئے کون سی تعدد استعمال کی جائے گی؟ اس دور کو مسئلہ خطی میل سے حل کیا جا سکتا ہے۔یوں باری باری ایک ایک منبع نصب کرتے ہوئے وقتی دائرہ کار میں جوابات حاصل کیے جائیں گے۔تمام جوابات کا مجموعہ درکار جواب ہو گا۔
...
شکل \حوالہ{شکل_بدلتا_آخری_ب} کو ترکیب جوڑ سے  حل کرتے ہوئے دونوں جوڑ کے دباو حاصل کریں۔
\begin{figure}
\centering
\begin{tikzpicture}
...
%======================
\ابتدا{سوال}
برقی دباو \عددی{v(t)=45\cos(100t-42^{\circ})\,\si{\volt}} کی تعدد، دوری عرصہ، زاویہ ہٹاو اور مؤثر دباو دریافت کریں۔
