\باب{مسئلے}
گزشتہ ابواب میں ہم نے ادوار میں مختلف مقامات پر دباؤ اور رو حاصل کرنے کے چند ترکیب دیکھے۔ایسا کرتے ہوئے ہم نے چند حقائق کا استعمال کیا جنہیں یہاں دوبارہ پیش کرتے ہیں۔

\حصہ{مساوی دور}
آپ جانتے ہیں کہ سلسلہ وار مزاحمتوں  کی جگہ ان کا مساوی مزاحمت نصب کرتے ہوئے ان کی رو حاصل کی جا سکتی ہے۔اسی طرح متوازی مزاحمتوں کی جگہ ان کا مساوی مزاحمت نصب کرتے ہوئے ان  پر دباؤ حاصل کیا جا سکتا ہے۔یہ عمل شکل \حوالہ{شکل_مسئلہ_مساوی_ادوار} میں دکھائے گئے ہیں۔اسی طرح سلسلہ وار منبع دباؤ کا مساوی اور متوازی منبع رو کا مساوی بالترتیب شکل-ج اور شکل-د میں دکھائے گئے ہیں۔یاد رہے کہ دو یا دو سے زیادہ منبع رو کو صرف اور صرف اس صورت سلسلہ وار جوڑا جا سکتا ہے جب تمام کی رو برابر ہو اور تمام  ایک ہی سمت میں ہوں۔ اسی طرح دو یا دو سے زیادہ منبع دباؤ کو صرف اور صرف اس صورت متوازی جوڑا جا سکتا ہے جب تمام منبع کے دباؤ برابر اور سمت ایک ہو۔
...
\end{tikzpicture}
\caption{سلسلہ وار منبع دباؤ کا مساوی منبع۔}
\end{subfigure}%
\begin{subfigure}{0.5\textwidth}
\centering
\begin{tikzpicture}
...
\حصہ{مسئلہ خطیّت}
برقی ادوار میں دباؤ اور رو درکار متغیرات ہیں۔ اس کتاب میں صرف ایسے ادوار پر غور کیا جائے گا جن میں دباؤ اور رو کا تعلق \اصطلاح{خطی}\فرہنگ{خطی}\حاشیہب{linear}\فرہنگ{linear} ہے۔انہیں خطی ادوار کہا جاتا ہے۔خطی ادوار میں ایک متغیرہ کو \عددی{n} گنا کرنے سے دوسرے متغیرات بھی \عددی{n} گنا ہو جاتے ہیں۔آئیں خطیت کی خاصیت سے دور حل کرنا دیکھیں۔

%==========================
\ابتدا{مثال}\شناخت{مثال_مسئلہ_خطیت_سے_دور_حل}
شکل \حوالہ{شکل_مسئلہ_خطیت_دور_حل} میں \عددی{\SI{60}{\ohm}} پر دباؤ معلوم کریں۔
\begin{figure}
\centering
\begin{tikzpicture}
...
حل:ہم اس دور کو با آسانی قوانین کرخوف سے حل کر سکتے ہیں۔آئیں اس دور کو خطیت کی خاصیت  کی مدد سے حل کریں۔اس ترکیب میں ہم درکار دباؤ کو \عددی{\SI{1}{\volt}} تصور کرتے ہوئے منبع رو کی قیمت دریافت کریں گے۔اس کے بعد خطیت کو استعمال کرتے ہوئے منبع رو کی اصل قیمت کے مطابقت سے درکار دباؤ حاصل کی جائے گی۔
...
جوابات:\عددی{I_M=\SI{74.5}{\milli\ampere}}، \عددی{I_0=\SI{2.685}{\milli\ampere}}
\انتہا{مشق}
%=====================
\ابتدا{مشق}\شناخت{مشق_مسئلہ_خطیت_ب}
شکل \حوالہ{شکل_مسئلہ_خطیت_ب} میں \عددی{V_R=\SI{2}{\volt}}  تصور کرتے ہوئے منبع دباؤ کی قیمت دریافت کریں۔خطیت کے استعمال سے \عددی{V_S=\SI{50}{\volt}} پر \عددی{V_R} دریافت کریں۔ 
\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l={$V_S$}]++(0,\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{8}{\kilo\ohm}$}]++(\x,0) to [resistor,l_={$\SI{2}{\kilo\ohm}$}]++(0,-\y) to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(-\x,0) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(-\x,0) to [short]++(-\x,0);
...
جوابات:\عددی{V_s=\tfrac{901}{25}\,\si{\volt}}، \عددی{V_0=\SI{2.77}{\volt}}
\انتہا{مشق}
%========================
\حصہ{مسئلہ خطی میل}
...
\end{align*}
انفرادی منبع سے دور میں مختلف مقامات پر نافذ دباؤ اور رو دریافت کرنے کی خاطر باری باری ایک ایک منبع کے علاوہ بقایا تمام منبع کے اثر کو ختم کرتے ہوئے دور کو حل کیا جاتا ہے۔منبع دباؤ کا اثر ختم کرنے کی خاطر اس کو قصر دور کیا جاتا ہے جبکہ منبع رو کے اثر کو ختم کرنے کی خاطر اس کو کھلے دور کیا جاتا ہے۔
...
اس حقیقت کو \اصطلاح{مسئلہ خطی میل}\فرہنگ{مسئلہ خطی میل}\حاشیہب{superposition}\فرہنگ{superposition} کہا جاتا ہے  جسے درج ذیل طریقے سے بیان کیا جا سکتا ہے۔

\ابتدا{قانون}
مسئلہ خطی میل کے تحت کسی بھی خطی دور، جس میں متعدد غیر تابع منبع دباؤ اور غیر تابع منبع رو پائے جاتے ہوں، میں  کسی بھی مقام پر نافذ دباؤ (رو)، تمام منبع کے انفرادی نافذ کردہ قیمتوں  کے مجموعے  کے برابر ہو گا۔
\انتہا{قانون}

آپ دیکھ سکتے ہیں کہ ہر منبع، دور میں یوں دباؤ اور رو نافذ کرتا ہے جیسے دور میں کوئی دوسرا منبع پایا ہی نا جاتا ہو۔

مسئلہ خطی میل کا عمومی ثبوت پیش کرتے ہیں۔صفحہ \حوالہصفحہ{مساوات_جوڑ_عمومی_مساوات_متعدد_منبع} پر مساوات \حوالہ{مساوات_جوڑ_عمومی_مساوات_متعدد_منبع} متعدد منبع دباؤ استعمال کرنے والے دور کی عمومی مساوات ہے جسے یہاں دوبارہ پیش کرتے ہیں۔
...
\end{align}
اگر \عددی{v_1} کے علاوہ تمام منبع دباؤ کو قصر دور کیا جائے تب ان کی قیمت \عددی{\SI{0}{\volt}} پُر کرتے ہوئے مساوات \حوالہ{مساوات_مسئلہ_عمومی_رو_حل} سے 
\begin{align*}
i'_1=g_{11} v_1
\end{align*}
حاصل ہوتا ہے۔ یہ صرف اور صرف \عددی{v_1} کی نافذ رو ہے۔اسی طرح \عددی{v_2} کے علاوہ تمام منبع کو قصر دور کرنے سے \عددی{i''_1=-g_{12}v_2}  نافذ ہوتی ہے۔اسی طرح بقایا منبع دباؤ کی نافذ  رو بھی حاصل کی جا سکتی ہیں۔آپ دیکھ سکتے ہیں کہ تمام  منبع کی انفرادی نافذ رو کا مجموعہ مساوات \حوالہ{مساوات_مسئلہ_عمومی_رو_حل} دیتی ہے۔

مساوات \حوالہ{مساوات_جوڑ_عمومی_مساوات_متعدد_منبع_دوبارہ} ان ادوار کو ظاہر کرتی ہے جن میں صرف منبع دباؤ پائے جاتے ہوں۔آپ اسی ترکیب کو استعمال کرتے ہوئے منبع رو کے اثرات کو بھی شامل کر سکتے ہیں۔

مسئلہ خطی میل  ان ادوار پر بھی لاگو ہوتا  ہے جن میں تابع منبع  پائے جاتے ہوں البتہ تابع منبع دباؤ کو قصر دور اور تابع منبع رو کو کھلے دور نہیں کیا جاتا۔ آئیں مسئلہ خطی میل کا استعمال چند مثالوں کی مدد سے سیکھیں۔

%================
\ابتدا{مثال}\شناخت{مثال_مسئلہ_متعدد_منبع_انفرادی_اثر_الف}
شکل \حوالہ{شکل_مسئلہ_مثال_منبع_انفرادی_اثر_الف} میں منبع دباؤ اور منبع رو کے انفرادی نافذ دباؤ حاصل کرتے ہوئے کُل \عددی{V_0} حاصل کریں۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
حل:منبع رو کو کھلے سر کرتے ہوئے شکل \حوالہ{شکل_مسئلہ_مثال_منبع_انفرادی_اثر_الف}-ب حاصل ہوتا ہے جس سے منبع دباؤ کا پیدا کردہ \عددی{V_{0}'} بذریعہ \اصطلاح{تقسیم دباؤ} کے کلیے سے حاصل کرتے ہیں۔
\begin{align*}
V_0'=10\left(\frac{4000}{1000+4000}\right)=\SI{8}{\volt}
\end{align*}
منبع دباؤ کو قصر دور کرتے ہوئے شکل \حوالہ{شکل_مسئلہ_مثال_منبع_انفرادی_اثر_الف}-پ حاصل ہوتا ہے جس سے منبع رو کا پیدا کردہ \عددی{V_{0}''} بذریعہ قانون اوہم حاصل کرتے ہیں۔
\begin{align*}
V_0''=\SI{-5}{\milli\ampere} \left(\frac{\SI{1}{\kilo\ohm}\times \SI{4}{\kilo\ohm}}{\SI{1}{\kilo\ohm}+\SI{4}{\kilo\ohm}}\right)=\SI{-4}{\volt}
\end{align*}
یوں دونوں منبع کی موجودگی میں کُل دباؤ \عددی{V_0'} اور \عددی{V_0''} کا مجموعہ ہو گا۔
\begin{align*}
V_0=V_0'+V_0''=8-4=\SI{4}{\volt}
\end{align*}
\انتہا{مثال}
%====================
\ابتدا{مثال}\شناخت{مثال_مسئلہ_منبع_دباو_منبع_رو_مجموعی_دباو}
شکل \حوالہ{شکل_مسئلہ_منبع_دباو_منبع_رو_مجموعی} میں منبع دباؤ اور منبع رو کو باری باری لیتے ہوئے \عددی{\SI{12}{\kilo\ohm}} پر نافذ دباؤ حاصل کرتے ہوئے دونوں منبع کی موجودگی میں کُل دباؤ حاصل کریں۔
\begin{figure}
\centering
\begin{tikzpicture}
...
\caption*{(ت)}
\end{subfigure}%
\caption{منبع دباؤ کا حصہ معلوم کرتے ہیں۔ }
\label{شکل_مسئلہ_مثال_منبع_دباو_حصہ}
\end{figure}

حل:شکل \حوالہ{شکل_مسئلہ_منبع_دباو_منبع_رو_مجموعی} میں منبع رو کو کھلا دور کرتے ہوئے شکل \حوالہ{شکل_مسئلہ_مثال_منبع_دباو_حصہ}-الف حاصل ہوتا ہے جس سے  منبع دباؤ سے پیدا کُل دباؤ کا حصہ دریافت کیا جائے گا۔شکل \حوالہ{شکل_مسئلہ_مثال_منبع_دباو_حصہ}-ب میں شکل کو قدر مختلف صورت دی گئی ہے۔چونکہ \عددی{\SI{4}{\kilo\ohm}} کا ایک سرا کہیں نہیں جڑا لہٰذا اس کا بقایا دور پر کوئی اثر نہیں ہو گا اور اسی لیے اس کو شکل-ب میں نہیں دکھایا گیا ہے۔
...
شکل-ب میں \عددی{\SI{12}{\kilo\ohm}} اور \عددی{\SI{8}{\kilo\ohm}} سلسلہ وار جڑے ہیں لہٰذا ان کا مساوی مزاحمت \عددی{\SI{20}{\kilo\ohm}} ہو گا۔شکل-پ میں ایسا دکھایا گیا ہے۔شکل-پ میں \عددی{\SI{20}{\kilo\ohm}} اور \عددی{\SI{10}{\kilo\ohm}} متوازی جڑے ہیں لہٰذا ان کا مساوی مزاحمت
 \عددی{\tfrac{\SI{20}{\kilo\ohm} \times \SI{10}{\kilo\ohm}}{\SI{20}{\kilo\ohm} +\SI{10}{\kilo\ohm} }=\tfrac{20}{3}\,\si{\kilo\ohm}} ہو گا جسے شکل-ت میں دکھایا گیا ہے جہاں سے تقسیم دباؤ کے کلیے سے
\begin{align*}
V'_1=4\left(\frac{\frac{20}{3} \, \si{\kilo\ohm}}{\SI{1}{\kilo\ohm}+\frac{20}{3} \, \si{\kilo\ohm}}\right) =\frac{80}{23}\,\si{\volt}
\end{align*}
لکھا جا سکتا ہے۔شکل-ب کو دیکھتے ہوئے تقسیم دباؤ کے کلیے سے درج ذیل حاصل ہوتا ہے۔
\begin{align*}
V'_0=\frac{80}{23}\left(\frac{\SI{12}{\kilo\ohm}}{\SI{12}{\kilo\ohm}+\SI{8}{\kilo\ohm}}\right)=\frac{48}{23}\, \si{\volt}
\end{align*}
آئیں اب منبع دباؤ کو قصر دور کرتے ہوئے حل کریں ۔شکل \حوالہ{شکل_مسئلہ_منبع_دباو_قصر_دور_کیا_گیا_ہے}-الف  میں منبع دباؤ کو قصر دور کیا گیا ہے۔آپ دیکھ سکتے ہیں کہ \عددی{\SI{1}{\kilo\ohm}} اور \عددی{\SI{10}{\kilo\ohm}} متوازی جڑے ہیں لہٰذا ان کی جگہ \عددی{\tfrac{\SI{1}{\kilo\ohm} \times \SI{10}{\kilo\ohm}}{\SI{1}{\kilo\ohm}+\SI{10}{\kilo\ohm}}=\tfrac{10}{11}\,\si{\kilo\ohm}} نصب کیا جا سکتا ہے۔ایسا ہی شکل-ب میں کیا گیا ہے جہاں \عددی{\tfrac{10}{11}\,\si{\kilo\ohm}} اور \عددی{\SI{8}{\kilo\ohm}} سلسلہ وار جڑے ہیں لہٰذا ان کی جگہ شکل-پ میں \عددی{\tfrac{98}{11}\,\si{\kilo\ohm}} نصب کیا گیا ہے۔شکل-ت میں متوازی جڑے \عددی{\tfrac{98}{11}\,\si{\kilo\ohm}} اور \عددی{\SI{12}{\kilo\ohm}} کی جگہ \عددی{\tfrac{588}{115}\,\si{\kilo\ohm}} نصب کیا گیا ہے۔اس شکل سے درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
...
جوابات:\عددی{\SI{4}{\volt}}، \عددی{\SI{-3}{\volt}}، \عددی{V_0=\SI{1}{\volt}}
\انتہا{مشق}
%=====================

\ابتدا{مشق}\شناخت{مشق_مسئلہ_متعدد_منبع_باری_باری_ب}
شکل \حوالہ{شکل_مسئلہ_باری_باری_ب} میں مسئلہ خطی میل کی مدد سے \عددی{V_0} دریافت کریں۔
...
جوابات:\عددی{\SI{10}{\volt}}، \عددی{\SI{5}{\volt}}، \عددی{\tfrac{8}{3}\,\si{\volt}}، \عددی{V_0=\tfrac{53}{3}\,\si{\volt}}
\انتہا{مشق}
%=====================


\ابتدا{مشق}\شناخت{مشق_مسئلہ_متعدد_منبع_باری_باری_پ}
شکل \حوالہ{شکل_مسئلہ_باری_باری_پ} کو مسئلہ خطی میل سے حل کرتے ہوئے  \عددی{I_0} دریافت کریں۔
...
جوابات:\عددی{\SI{0.201}{\milli\ampere}}، \عددی{\SI{0.309}{\milli\ampere}}، \عددی{\SI{-0.165}{\milli\ampere}}، \عددی{I_0=\SI{0.345}{\milli\ampere}}
\انتہا{مشق}
%=====================
\ابتدا{مشق}\شناخت{مشق_مسئلہ_منبع_کے_گروہ_کی_رو}
شکل \حوالہ{شکل_مسئلہ_میل_منبع_کے_گروہ} میں \عددی{\SI{6}{\volt}} منبع کے اثر کو ختم کرتے ہوئے \عددی{\SI{10}{\volt}} اور \عددی{\SI{3}{\milli\ampere}} منبع کا مجموعی نافذ رو  \عددی{i'} حاصل کریں۔اب اکیلے \عددی{\SI{6}{\volt}} منبع کا اسی مزاحمت میں نافذ رو \عددی{i''} دریافت کریں۔دونوں جوابات سے تینوں منبع سے پیدا مجموعی رو \عددی{i=i'+i''} دریافت کریں۔
\begin{figure}
\centering
\centering
\begin{subfigure}{1\textwidth}
...
\label{شکل_مسئلہ_میل_منبع_کے_گروہ}
\end{figure}

جوابات:شکل \حوالہ{شکل_مسئلہ_میل_منبع_کے_گروہ}-ب سے \عددی{i'=\tfrac{25}{9} \, \si{\milli\ampere}} اور شکل \حوالہ{شکل_مسئلہ_میل_منبع_کے_گروہ}-پ سے \عددی{i''=-\tfrac{7}{9}\, \si{\milli\ampere}} حاصل ہوتا ہے۔یوں شکل-الف میں \عددی{i=\SI{2}{\milli\ampere}} حاصل ہوتا ہے۔
\انتہا{مشق}
%=================
\حصہ{مساوی ادوار}
شکل \حوالہ{شکل_مسئلہ_مساوی_دور_الف} میں دو عدد ادوار نقطہ دار لکیر میں بند دکھائے گئے ہیں۔تصور کریں کہ نقطہ دار لکیر بند ڈبے کو ظاہر کرتی ہے جس کے اندر دیکھنا ممکن نہیں ہے۔ہم بند ڈبے سے باہر نکلتی برقی سروں پر مختلف مزاحمت یا ادوار نصب کرتے ہوئے معلوم کرنا چاہتے ہیں کہ ان کے اندر کیا نصب ہے۔
\begin{figure}
\centering
\begin{subfigure}{0.5\textwidth}
\centering
...
شکل \حوالہ{شکل_مسئلہ_مساوی_دور_الف}-الف اور شکل \حوالہ{شکل_مسئلہ_مساوی_دور_الف}-ب کے  برقی سروں پر \عددی{\SI{8}{\kilo\ohm}} مزاحمت نصب کرتے ہوئے برقی سروں پر دباؤ اور رو حاصل کریں۔بند ڈبے کو نہیں دکھایا گیا تا کہ شکل صاف ستھری نظر آئے۔
...
\end{figure}
شکل \حوالہ{شکل_مسئلہ_مساوی_دور_ب}-الف میں \عددی{\SI{4}{\kilo\ohm} \parallel \SI{8}{\kilo\ohm}=\tfrac{8}{3} \, \si{\kilo\ohm}} لیتے ہوئے تقسیم دباؤ کے کلیے سے 
\begin{align*}
...
شکل \حوالہ{شکل_مسئلہ_مساوی_دور_الف} کے  برقی سروں پر سلسلہ وار جڑے منبع دباؤ اور مزاحمت نصب کرتے ہوئے شکل \حوالہ{شکل_مسئلہ_مساوی_دور_پ} میں دکھایا گیا ہے۔انہیں حل کریں۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
حل:نچلی جوڑ کو زمین چنتے ہوئے بالائی جوڑ پر دباؤ \عددی{v_0} لکھی جائے گی۔یوں  شکل \حوالہ{شکل_مسئلہ_مساوی_دور_پ}-الف کے بالائی جوڑ پر درج ذیل کرخوف مساوات رو لکھی جا سکتی ہے
\begin{align*}
...
v_3&=\SI{6}{\volt}
\end{align*}
دباؤ جوڑ جانتے ہوئے تمام شاخوں کی رو دریافت کی جا سکتی ہے۔آئیں اس دور کو نقطہ دار لکیر پر دو ٹکڑوں میں تقسیم کرتے ہیں۔شکل \حوالہ{شکل_مسئلہ_تھونن_سمجھنا_دور}-ب میں بائیں حصے کو دکھایا گیا ہے جہاں جوڑ \عددی{v_3} پر \عددی{\SI{6}{\volt}} منبع دباؤ نصب کیا گیا ہے۔ اس کو حل کرنے کی خاطر کرخوف قانون رو سے درج ذیل لکھتے ہیں
\begin{align*}
\frac{v_1-10}{4000}+\frac{v_1}{3000}+\frac{v_1-v_2}{4000}&=0\\
...
v_2&=\SI{10}{\volt}
\end{align*}
حاصل ہوتے ہیں۔آپ نے دیکھا کہ شکل-ب کے  دباؤ جوڑ بالکل تبدیل نہیں ہوئے لہٰذا اس میں تمام مقامات پر رو بھی وہی ہو گی جو شکل-الف میں تھی۔

شکل \حوالہ{شکل_مسئلہ_تھونن_سمجھنا_دور}-الف میں نقطہ دار لکیر کے بائیں حصے پر لکیر کے دائیں جانب دور کا اثر صرف اور صرف جوڑ \عددی{v_3} کے ذریعہ ہوتا ہے۔یوں جیسا شکل-ب میں کیا گیا، اگر جوڑ \عددی{v_3} پر دباؤ اسی قیمت پر رکھا جائے جو لکیر کے دائیں جانب دور کے نصب کرنے سے حاصل ہوتا ہے، تب  لکیر کے بائیں جانب دور کے متغیرات جوں کے توں رہتے ہیں۔  

شکل \حوالہ{شکل_مسئلہ_تھونن_سمجھنا_دور}-ب میں رو  \عددی{i} کو  مسئلہ خطی میل سے حاصل کیا جا سکتا ہے۔آپ مشق \حوالہ{مشق_مسئلہ_منبع_کے_گروہ_کی_رو} میں اس دور کو مسئلہ خطی میل کی مدد سے حل کر چکے ہیں۔اسی مشق کے شکل \حوالہ{شکل_مسئلہ_میل_منبع_کے_گروہ}-پ میں بقایا منبع کے اثر کو ختم کرتے ہوئے \عددی{\SI{6}{\volt}} کو صرف مزاحمت نظر آتے ہیں۔آئیں شکل-پ میں دیے دور کا مساوی مزاحمت حاصل کرتے ہیں۔منبع سے دور ترین نقطے سے شروع کرتے ہیں جہاں چار کلو اوہم اور تین کلو اوہم  متوازی \عددی{\SI{4}{\kilo\ohm} \parallel \SI{3}{\kilo\ohm}} جڑے ہیں۔متوازی جڑے مزاحمت اذ خود سلسلہ وار جڑے \عددی{\SI{2}{\kilo\ohm}} اور \عددی{\SI{4}{\kilo\ohm}} کے ساتھ سلسلہ وار پائے جاتے ہیں لہٰذا ان تمام کا مجموعی مساوی مزاحمت
\begin{align*}
...
شکل \حوالہ{شکل_مسئلہ_تھونن_عمومی_دور}-الف میں عمومی ڈبہ دور دکھایا گیا ہے۔اس کو دو حصوں میں تقسیم کرتے ہوئے شکل-ب حاصل ہوتا ہے۔شکل-ب میں بائیں حصے کے مساوی تھونن دور اور مساوی نارٹن دور حاصل کیے جائیں گے۔بایاں حصہ خطی ہونا ضروری ہے۔دایاں حصہ خطی یا غیر خطی ہو سکتا ہے۔دائیں حصے کو برقی بوجھ تصور کیا جائے گا۔یہ حصے دو تاروں سے آپس میں جڑے ہیں۔ان تاروں کے مابین \عددی{v_0} دباؤ پایا جاتا ہے جبکہ بوجھ کو رو \عددی{i} مہیا کی جاتی ہے۔اگر شکل-ب میں بائیں ڈبے دور کی جگہ اس کا مساوی تھونن دور یا مساوی نارٹن دور نصب کرنے سے \عددی{v_0} اور \عددی{i} کی قیمتوں پر فرق نہیں پڑے تب دائیں ڈبے کی نقطہ نظر سے دور میں کوئی تبدیلی رو نما نہیں ہوئی ہے لہٰذا اس کے لیے بایاں ڈبے کا دور اور مساوی تھونن (یا مساوی نارٹن) دور یک برابر ہیں۔
...
شکل-پ میں دائیں حصے کی جگہ  منبع دباؤ نصب کیا گیا ہے جس کا دباؤ \عددی{v_0} ہے۔ 
...
\caption*{(ب)}
\end{subfigure}%
\caption{رو کو مسئلہ خطی میل سے دو حصوں میں تقسیم کیا جا سکتا ہے۔}
...
\end{tikzpicture}
\caption{کھلے دور سروں پر صفر رو اور تھونن دباؤ پائی جاتی ہے۔}
...
شکل \حوالہ{شکل_مسئلہ_تھونن_نارٹن}-الف کی کرخوف مساوات دباؤ اور شکل \حوالہ{شکل_مسئلہ_تھونن_نارٹن}-ب کے بالائی جوڑ پر کرخوف مساوات رو درج ذیل ہیں۔
\begin{align*}
...
دور میں منبع دباؤ کو قصر دور اور منبع رو کو کھلے دور کرتے ہوئے شکل-پ حاصل ہوتا ہے جہاں سے
\begin{align*}
R_{\text{تھونن}}=\SI{4}{\kilo\ohm}
\end{align*}
لکھا جا سکتا ہے۔یوں شکل-ب کی جگہ اس کا مساوی تھونن دور نصب کرتے ہوئے شکل-الف کی جگہ شکل-ت حاصل ہوتا ہے جسے دیکھتے ہوئے  تقسیم دباؤ کے کلیے سے بوجھ پر دباؤ  درج ذیل لکھا جا سکتا ہے۔
\begin{align}
...
\end{align*}
ہے  جس میں \عددی{\tfrac{5}{4} \, \si{\milli\ampere}} گزرنے سے دباؤ
\begin{align*}
...
شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_ب}-الف میں \عددی{\SI{3}{\kilo\ohm}} کو بوجھ تصور کریں۔بار بار تھونن سے نارٹن اور نارٹن سے تھونن مساوی دور حاصل کرتے ہوئے بقایا دور کا  تھونن مساوی حاصل کرتے ہوئے بوجھ پر دباؤ حاصل کریں۔
...
\draw(3*\x,0)node[below]{$f$} to [resistor,*-*,l={$\SI{3}{\kilo\ohm}$}]++(0,\y)node[above]{$e$};
\draw(3*\x+\x/2,\y/2)node{$\begin{aligned} &+ \\ &v_0 \\ &- \end{aligned}$};
\draw(3*\x+\x/2+\x,\y/2)node[left]{(پ)};
\end{tikzpicture}
%\caption*{(پ)}
\end{subfigure}
\begin{subfigure}{1\textwidth}
...
\draw(3*\x,0)node[below]{$f$} to [resistor,*-*,l={$\SI{3}{\kilo\ohm}$}]++(0,\y)node[above]{$e$};
\draw(3*\x+\x/2,\y/2)node{$\begin{aligned} &+ \\ &v_0 \\ &- \end{aligned}$};
\draw(3*\x+\x/2+\x,\y/2)node[left]{(ت)};
\end{tikzpicture}
%\caption*{(ت)}
\end{subfigure}
\begin{subfigure}{1\textwidth}
...
\draw(3*\x,0)node[below]{$f$} to [resistor,*-*,l={$\SI{3}{\kilo\ohm}$}]++(0,\y)node[above]{$e$};
\draw(3*\x+\x/2,\y/2)node{$\begin{aligned} &+ \\ &v_0 \\ &- \end{aligned}$};
\draw(3*\x+\x/2+\x,\y/2)node[left]{(ٹ)};
\end{tikzpicture}
%\caption*{(ٹ)}
\end{subfigure}
...
\end{align*}
حاصل کیا جاتا ہے۔شکل-پ میں \عددی{\SI{4}{\milli\ampere}}  اور  \عددی{\tfrac{12}{7}\,\si{\kilo\ohm}} کے نارٹن دور کی جگہ \عددی{\tfrac{48}{7} \, \si{\volt}} اور \عددی{\tfrac{12}{7}\,\si{\kilo\ohm}} کا تھونن دور نصب کرنے سے شکل-ت حاصل ہوتا ہے۔شکل-ت میں سلسلہ وار جڑے \عددی{\tfrac{12}{7}\,\si{\kilo\ohm}} اور \عددی{\SI{1}{\kilo\ohm}} کی جگہ ان کا مساوی \عددی{\tfrac{19}{7}\,\si{\kilo\ohm}} نصب کرنے سے شکل-ٹ حاصل ہوتا ہے۔

شکل-ٹ میں \عددی{\tfrac{19}{7}\,\si{\kilo\ohm}} اور  \عددی{\tfrac{48}{7} \, \si{\volt}} مل کر تھونن دور بناتے ہیں جن کی جگہ نارٹن دور نصب کرنے کی غرض سے 
\begin{align*}
...
\end{align*}
کے برابر ہے۔شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_پ}-ب میں متوازی منبع کی جگہ ان کی مجموعی قیمت کا منبع نصب کیا گیا ہے۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
\end{subfigure}%
\caption*{(ت)}
\caption{مثال \حوالہ{مثال_مسئلہ_تھونن_ت} حل کرتے ہوئے حاصل کیے گئے ادوار۔}
\label{شکل_مثال_تھونن_سے_نارٹن_پ}
\end{figure}

شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_پ}-ب میں \عددی{\frac{124}{19}\,\si{\milli\ampere}} اور \عددی{\tfrac{19}{7}\,\si{\kilo\ohm}} نارٹن دور  کی جگہ ان کا مساوی تھونن دور  نصب کرنے سے شکل-پ حاصل ہوتا ہے جس میں \عددی{\tfrac{19}{7}\,\si{\kilo\ohm}} اور \عددی{\SI{4}{\kilo\ohm}} سلسلہ وار جڑے ہیں جن کا مساوی \عددی{\tfrac{47}{7}\,\si{\kilo\ohm}} ہے۔شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_پ}-ت میں یہی مساوی مزاحمت دکھایا گیا ہے۔

شکل-ت میں \عددی{\SI{3}{\kilo\ohm}} بوجھ ہے جبکہ بقایا تھونن مساوی ہے۔تقسیم دباؤ سے بوجھ پر دباؤ درج ذیل حاصل ہوتا ہے۔
\begin{align*}
...
\caption*{(ٹ)}
\end{subfigure}%
\caption{مثال \حوالہ{مثال_مسئلہ_تھونن_ٹ} حل کرتے ہوئے حاصل کیے گئے ادوار۔}
...
\end{align*}
ملتا ہے۔یوں شکل-ب کا مساوی نارٹن دور شکل-پ حاصل ہوتا ہے جسے شکل-الف میں \عددی{cd} کے بائیں جانب دور کی جگہ نصب کرنے سے شکل-ت ملتا ہے۔شکل-ت میں دو عدد منبع رو متوازی جڑی ہیں جن کی جگہ ایک عدد
\begin{align*}
\frac{48}{19} \, \si{\milli\ampere}+\SI{4}{\milli\ampere}=\frac{124}{19} \, \si{\milli\ampere}
\end{align*}
 \عددی{\SI{8}{\milli\ampere}} کی منبع نصب کی جا سکتی ہے  جس سے شکل-ٹ حاصل ہوتا ہے۔شکل-ٹ میں سلسلہ وار جڑے \عددی{\SI{4}{\kilo\ohm}} اور \عددی{\SI{3}{\kilo\ohm}} از خود \عددی{\tfrac{19}{7} \, \si{\kilo\ohm}} کے متوازی ہے۔یوں سلسلہ وار مزاحمتوں میں رو کو تقسیم رو کے کلیے سے درج ذیل لکھا جا سکتا ہے
\begin{align*}
...
\end{align*}
جسے شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_ٹ}-ٹ میں دکھایا گیا ہے۔تین کلو  بوجھ پر دباؤ درج ذیل ہے۔
\begin{align*}
v_{\text{کھلا}}=\frac{31}{17} \, \si{\milli\ampere} \times \SI{3}{\kilo\ohm}=\frac{93}{17} \, \si{\volt}
\end{align*}
آخر میں مسئلہ اتنا سادہ بن چکا تھا کہ تقسیم رو اور اوہم کے قانون سے دباؤ حاصل کیا گیا۔آپ دیکھ سکتے ہیں کہ بوجھ پر دباؤ  جلد حاصل ہوا لہٰذا مسئلے کو دیکھ کر فیصلہ کریں کہ کہاں سے دور کو ٹکڑے کرتے ہوئے حل کرنا ہے۔
\انتہا{مثال}
%=========================
...
حل:آٹھ کلو اوہم کی مزاحمت کو بوجھ تصور کرتے ہوئے بقایا دور کا نارٹن مساوی حاصل کرتے ہیں۔بوجھ کو بقایا دور سے علیحدہ کرتے  ہوئے تھونن مزاحمت حاصل کرنے کی خاطر منبع رو کو کھلے دور اور منبع دباؤ کو قصر دور کرتے ہوئے شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_ث}-ب  حاصل ہوتا ہے۔اس کو دیکھ کر 
\begin{align*}
...
\end{align*}
حاصل ہوتا ہے۔تھونن مزاحمت اور نارٹن رو جانتے ہوئے   \عددی{\SI{8}{\kilo\ohm}} بوجھ کے علاوہ \حوالہ{شکل_مثال_تھونن_سے_نارٹن_ث}-الف کے بقایا دور کا مساوی نارٹن  دور شکل \حوالہ{شکل_مثال_تھونن_سے_نارٹن_ث}-ت میں دکھایا گیا ہے جہاں نارٹن رو کی قیمت منفی ہونے کی  وجہ سے اسے الٹ سمت میں دکھایا گیا ہے۔نارٹن مساوی دور کے ساتھ \عددی{\SI{8}{\kilo\ohm}} بوجھ جوڑنے سے شکل-ٹ حاصل ہوتی ہے۔اس شکل کو دیکھ کر درکار دباؤ درج ذیل لکھی جا سکتی ہے۔
\begin{align*}
...
شکل \حوالہ{شکل_مسئلہ_مشق_تھونن_الف} میں دور دکھایا گیا ہے جسے  مسئلہ تھونن سے حل کرتے ہوئے \عددی{V_0} حاصل کریں۔(دائیں جانب نصب \عددی{\SI{2}{\kilo\ohm}} سے دیکھتے ہوئے بقایا دور کا تھونن دور حاصل کرتے ہوئے حل کریں۔)
\begin{figure}
\centering
\begin{tikzpicture}
...
جوابات:\عددی{R_{\text{تھونن}}=\SI{10}{\kilo\ohm}}، \عددی{V_{\text{تھونن}}=\SI{30}{\volt}}، \عددی{V_0=\SI{5}{\volt}}
\انتہا{مشق}
%==========================
\ابتدا{مشق}\شناخت{مشق_مسئلہ_تھونن_ب}
شکل \حوالہ{شکل_مسئلہ_مشق_تھونن_ب} کو تھونن مساوی دور سے حل کرتے ہوئے \عددی{V_0} حاصل کریں۔(\SI{4}{\kilo\ohm} مزاحمت کے ساتھ جڑی دور کا تھونن مساوی حاصل کرتے ہوئے حل کریں۔)
\begin{figure}
\centering
\begin{tikzpicture}
...
جوابات:\عددی{R_{\text{تھونن}}=\tfrac{86}{21}\,\si{\kilo\ohm}}، \عددی{V_{\text{تھونن}}=\tfrac{100}{7}\,\si{\volt}}، \عددی{V_0=\tfrac{120}{17}\,\si{\volt}}
\انتہا{مشق}
%===================
\ابتدا{مشق}\شناخت{مشق_مسئلہ_تھونن_پ}
مسئلہ نارٹن کی مدد سے شکل \حوالہ{شکل_مسئلہ_مشق_تھونن_پ} میں \عددی{I_0} حاصل کریں۔(دائیں جانب نصب \SI{4}{\kilo\ohm} مزاحمت  کے ساتھ منسلک دور کا نارٹن مساوی حاصل کرتے ہوئے حل کریں۔)
\begin{figure}
\centering
\begin{tikzpicture}
...
جوابات:\عددی{R_{\text{تھونن}}=\SI{14}{\kilo\ohm}}، \عددی{I_{\text{نارٹن}}=\tfrac{4}{7}\,\si{\milli\ampere}}، \عددی{I_0=\tfrac{4}{9}\,\si{\milli\ampere}}
\انتہا{مشق}
%==================

\حصہ{تابع منبع استعمال کرنے والے ادوار}
صرف تابع منبع استعمال کرنے والے ادوار کا تھونن یا نارٹن مساوی دور صرف \عددی{R_{\text{تھونن}}} ہوتا ہے۔ایسے ادوار میں چونکہ غیر تابع منبع نہیں پایا جاتا لہٰذا یہ از خود طاقت مہیا نہیں کر سکتے اور یوں ان سے تھونن دباؤ اور نارٹن رو صفر حاصل ہوتی ہیں۔تابع منبع استعمال کرنے والے  ادوار کا تھونن مزاحمت حاصل کرتے ہوئے اندرونی تابع منبع دباؤ کو قصر دور اور اندرونی تابع منبع رو کو کھلے دور نہیں کیا جاتا۔ان ادوار کے برقی سروں پر پیمائشی دباؤ \عددی{v_p} مہیا کرتے ہوئے انہیں سروں پر رو \عددی{i_p} حاصل کی جاتی ہے۔مزاحمت کی تعریف سے تھونن مزاحمت درج ذیل لکھی جاتی ہے۔
\begin{align}
...
شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_الف}-الف میں تابع منبع دباؤ پایا جاتا ہے۔اس دور کا مساوی تھونن دور حاصل کریں۔
...
حل:شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_الف}-ب میں برقی سروں \عددی{AB} پر پیمائشی دباؤ لاگو کرتے ہوئے \عددی{i_p} حاصل کرتے ہیں۔پیمائشی دباؤ کی قیمت کچھ بھی چنی جا سکتی ہے۔ہم نے  \عددی{v_p=\SI{1}{\volt}} چنا ہے۔نچلی جوڑ کو زمین چنتے ہوئے  درج ذیل مساوات لکھے جا سکتے ہیں
\begin{align*}
...
حل:اس دور میں صرف تابع منبع پایا جاتا ہے اور ہم توقع کرتے ہیں کہ نارٹن رو یا تھونن دباؤ صفر حاصل ہو گا۔ آئیں دیکھیں کہ آیا ہماری توقع درست ہے۔شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_ب}-الف میں نچلے جوڑ کو زمین تصور کرتے  ہوئے جوڑ \عددی{V_1} پر کرخوف قانون رو سے درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
...
V_1=\SI{0}{\volt}
\end{align*}
حاصل ہوتا ہے۔تقسیم دباؤ کے کلیے سے 
\begin{align*}
V_{\text{تھونن}}=\left(\frac{1000}{1000+4000}\right) V_1=\SI{0}{\volt}
\end{align*}
حاصل ہوتا ہے۔چونکہ تھونن دباؤ صفر ہے لہٰذا مسئلہ تبادلہ منبع کے تحت نارٹن رو بھی صفر ہو گی۔


دور کی تھونن مزاحمت حاصل کرنے کی خاطر برقی سروں پر بیرونی منبع نصب کرنا ہو گا۔شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_ب}-ب میں برقی سروں پر
 \عددی{i_p=\SI{1}{\milli\ampere}} کا پیمائشی رو نصب کیا گیا ہے۔برقی سروں پر پیمائشی دباؤ \عددی{v_p} جانتے ہوئے  تھونن مزاحمت حاصل کی جا سکتی ہے۔
...
گزشتہ مثال کے دور کو سلسلہ وار جڑے بیرونی منبع  اور مزاحمت سے طاقت مہیا کی جاتی ہے۔شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_پ} میں اسے دکھایا گیا ہے۔برقی سروں پر دباؤ \عددی{v_0} اور رو \عددی{i_0} حاصل کریں۔اب گزشتہ مثال کے دور کی جگہ اس کا مساوی تھونن دور نصب کرتے ہوئے دوبارہ حل کریں۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
آئیں اب تھونن مساوی دور کی مدد سے اسی کو دوبارہ حل کریں۔گزشتہ مثال میں \عددی{v_{\text{تھونن}}=\SI{0}{\volt}} اور \عددی{R_{\text{تھونن}}=\frac{8}{5} \, \si{\kilo\ohm}} حاصل کیے گئے۔تھونن مساوی دور استعمال کرتے ہوئے شکل \حوالہ{شکل_مسئلہ_مثال_تھونن_تابع-منبع_پ}-ب حاصل ہوتا ہے جہاں قانون اوہم کی مدد سے
\begin{align*}
i_0&=\frac{\SI{6}{\volt}}{\frac{8}{5} \, \si{\kilo\ohm}+\SI{2}{\kilo\ohm}}=\frac{5}{3}\,\si{\milli\ampere}
\end{align*}
اور تقسیم دباؤ کے کلیے سے
\begin{align*}
...
ان ادوار میں \عددی{v_{\text{کھلا}}} اور \عددی{i_{\text{قصر}}} حاصل کرتے ہوئے \عددی{R_{\text{تھونن}}} حاصل کیا جاتا ہے۔یاد رہے کہ دور کو دو ٹکڑوں میں تقسیم کرتے ہوئے تابع منبع اور اس کا قابو متغیر علیحدہ نہیں کیے جا سکتے ہیں۔
...
حل:دور کے ٹکڑے کرتے ہوئے تھونن مساوی دور حاصل کرتے ہیں۔یاد رہے کہ تابع منبع  اور اس کا قابو متغیر کو علیحدہ نہیں کیا جا سکتا ہے لہٰذا قابو منبع رو اور \عددی{\SI{8}{\kilo\ohm}} مزاحمت کو علیحدہ نہیں کیا جا سکتا ہے۔یوں دور کو ٹکڑے کرتے ہوئے، برقی سروں سے دور ترین نقطے سے شروع کرتے ہوئے  کم از کم اتنے اجزاء شامل کیے جائیں گے کہ قابو منبع سے لے کر تابع متغیر تک تمام اس میں موجود ہوں۔شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_مثال_الف}-ب میں ایسا ٹکڑا دکھایا گیا ہے جہاں قابو متغیر کو \عددی{I'_x} کہا گیا ہے۔بالائی  مخلوط جوڑ پر کرخوف مساوات رو لکھتے ہیں
\begin{align*}
...
شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_مثال_الف}-ب میں قصر دور رو حاصل کرنے کی خاطر اس کے برقی سروں کو قصر دور کرتے ہوئے شکل-پ حاصل کیا جاتا ہے جس میں \عددی{I''_x=0} کی  وجہ سے  قابو منبع کی رو بھی صفر ہو گی۔ان حقائق کو مد نظر رکھتے ہوئے شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_مثال_الف}-ت حاصل ہوتا ہے جسے دیکھ کر 
\begin{align*}
I_{\text{قصر}}=\frac{6}{4000}=\frac{3}{2}\, \si{\milli\ampere}
\end{align*}
لکھا جا سکتا ہے۔تھونن دباؤ اور نارٹن رو استعمال کرتے ہوئے  تھونن مزاحمت درج ذیل حاصل ہوتا ہے۔
\begin{align*}
R_{\text{تھونن}}=\frac{V_{\text{کھلا}}}{I_{\text{قصر}}}  = \frac{\frac{12}{7} \, \si{\volt}}{\frac{3}{2} \, \si{\milli\ampere}}=\frac{8}{7} \, \si{\kilo\ohm}
\end{align*}
شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_مثال_الف}-ب کی جگہ تھونن مساوی دور نصب کرتے ہوئے شکل-الف سے شکل-ٹ حاصل ہوتا ہے جس سے تقسیم دباؤ کے کلیے سے 
\begin{align*}
...
\end{align*}
لکھا جا سکتا ہے۔تابع منبع کی موجودگی کی  وجہ سے  تھونن مزاحمت حاصل کرنے کی خاطر قصر دور رو درکار ہو گی۔شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_مثال_ب}-ب کے برقی سر قصر دور کرتے ہوئے  شکل-پ حاصل ہوتا ہے جسے شکل-ت کی طرز پر بنایا جا سکتا ہے۔شکل-ت میں \عددی{I_{\text{قصر}}} کی نشاندہی کرنا قدر مشکل کام ہے البتہ اس سے \عددی{I_a} نہایت آسانی سے
\begin{align*}
...
\end{align*}
حاصل ہوتا ہے جن کی مدد سے شکل-ب کا تھونن مساوی دور حاصل کیا جا سکتا ہے جسے شکل-الف میں پُر کرنے سے شکل-ٹ حاصل ہوتا ہے۔شکل-ٹ سے تقسیم دباؤ کے کلیے سے درج ذیل لکھا جا سکتا ہے۔
\begin{align*}
V_0=-14 \left(\frac{\SI{8}{\kilo\ohm}}{\SI{8}{\kilo\ohm}+\SI{6}{\kilo\ohm}}\right)=\SI{-8}{\volt}
\end{align*}
\انتہا{مثال}
%=====================
\حصہء{تھونن ادوار حل کرنے کا قدم بہ قدم طریقہ}
برقی بوجھ کو ہٹا کر کھلے سروں کے مابین  دباؤ \عددی{V_{\text{کھلا}}} حاصل کریں۔تھونن دباؤ حاصل کرتے وقت ادوار حل کرنے کے تمام طریقے بروئے کار لائے جا سکتے ہیں۔

کھلے سروں کے مابین تھونن مزاحمت حاصل کریں۔یہ مزاحمت حاصل کرتے وقت تین اقسام کے ادوار کا سامنا کرنا پڑ سکتا ہے۔پہلی قسم  کے ادوار میں صرف غیر تابع منبع استعمال کیا جاتا ہے۔ان ادوار میں منبع دباؤ کو قصر دور اور منبع رو کو کھلے دور کرتے ہوئے  تھونن مزاحمت حاصل کی جاتی ہے۔ دوسری قسم کے ادوار میں صرف تابع منبع پائے جاتے ہیں۔ان ادوار کے برقی سروں پر پیمائشی منبع دباؤ یا پیمائشی منبع رو نصب کرتے ہوئے برقی سروں پر دباؤ اور رو حاصل کی جاتی ہے۔برقی سروں کا دباؤ تقسیم رو سے تھونن مزاحمت حاصل ہوتی ہے۔تیسری قسم کے ادوار میں تابع منبع اور غیر تابع منبع دونوں پائے جاتے ہیں۔ان اقسام کے ادوار میں برقی سروں کو آپس میں قصر دور کرتے ہوئے قصر دور رو حاصل کی جاتی ہے۔کھلے دور دباؤ تقسیم قصر دور رو کی شرح تھونن مزاحمت دیتی ہے۔

سلسلہ وار جڑے کھلے دور دباؤ \عددی{V_{\text{کھلا}}} اور تھونن مزاحمت \عددی{R_{\text{تھونن}}} کے ساتھ بوجھ جوڑتے ہوئے بوجھ پر دباؤ اور اس کی رو حاصل کی جاتی ہے۔ 

مسئلہ نارٹن کے استعمال میں بالکل اسی طرح چلتے ہوئے آخری قدم پر متوازی جڑے قصر دور رو \عددی{I_{\text{قصر}}} اور تھونن مزاحمت \عددی{R_{\text{تھونن}}} کے ساتھ بوجھ جوڑتے ہوئے بوجھ کا دباؤ اور رو حاصل کی جاتی ہیں۔ 
...
جواب:\عددی{V_0=-\tfrac{52}{5}\,\si{\volt}}
\انتہا{مشق}
%====================


\ابتدا{مشق}\شناخت{مشق_مسئلہ_تابع_غیر_تابع_پ}
شکل \حوالہ{شکل_مسئلہ_تابع_غیر_تابع_پ} میں مسئلہ تھونن کی مدد سے  منبع دباؤ کی فراہم کردہ طاقت حاصل کریں۔
\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [short]++(3*\x,0);
...
جواب:\عددی{\SI{59.6}{\milli\watt}}
\انتہا{مشق}
%====================
\حصہ{زیادہ سے زیادہ طاقت منتقل کرنے کا مسئلہ}\شناخت{حصہ_مسئلے_زیادہ_سے_زیادہ_طاقت_منتقل}
کسی بھی دور کے برقی سروں پر بوجھ لادنے سے  بوجھ میں طاقت منتقل ہوتی ہے۔آپ جانتے ہیں کہ ہر ممکنہ دور کا تھونن مساوی دور حاصل کیا جا سکتا ہے لہٰذا  اس مسئلے کو شکل \حوالہ{شکل_مسئلہ_بوجھ_کو_طاقت_کی_منتقلی} سے ظاہر کیا جا سکتا ہے۔
\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [short,-o]++(\x+\x/4,0);
\draw(0,0) to [american voltage source,l={$V_{\text{تھونن}}$}]++(0,\y) to [resistor,i={$i$},l={$R_{\text{تھونن}}$}]++(\x,0) to [short,-o]++(\x/4,0) to [short,o-]++(\x/4,0) to [resistor,l={$R_{\text{بوجھ}}$}]++(0,-\y) to [short,-o]++(-\x/4,0);
\end{tikzpicture}
...
\end{align*}
طاقت منتقل ہو گی۔آئیں جانتے ہیں کہ کس قیمت کے \عددی{R_{\text{بوجھ}}} کو زیادہ سے زیادہ طاقت  منتقل ہو گی۔یہ جاننے کے لیے درج بالا مساوات کے تفرق کو صفر کے برابر پُر کرتے ہوئے  \عددی{R_{\text{بوجھ}}} کی درکار قیمت حاصل کرتے ہیں۔
\begin{align*}
...
اس سے
\begin{align}
R_{\text{بوجھ}}=R_{\text{تھونن}} \quad \quad \text{\RL{بوجھ کو زیادہ سے زیادہ طاقت کی منتقلی کی شرط}}
\end{align}
...
%===============
\ابتدا{مثال}\شناخت{مثال_مسئلہ_طاقت_منتقل_الف}
شکل \حوالہ{شکل_مسئلہ_طاقت_منتقلی_الف} میں مزاحمت بوجھ کی وہ قیمت دریافت کریں جس میں زیادہ سے زیادہ طاقت منتقل ہو گی۔اس طاقت کا تخمینہ لگائیں۔مزاحمت بوجھ کی قیمت \عددی{\SI{10}{\percent}} کم اور زیادہ ہونے کی صورت میں اسے کتنی طاقت منتقل ہوتی ہے۔
...
\centering
\begin{tikzpicture}
\draw(0,0) to [short,-o]++(2*\x+\x/4,0);
\draw(0,0) to [american voltage source,l={$\SI{10}{\volt}$}]++(0,\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{6}{\kilo\ohm}$}]++(\x,0) to [short,-o]++(\x/4,0);
\draw(\x,0) to [resistor,*-*,l_={$\SI{8}{\kilo\ohm}$}]++(0,\y);
\draw(0,\y) to [short,*-]++(0,\y) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(2*\x,0) to [short,-*]++(0,-\y);
\draw(2*\x+\x/4,0) to [short,o-]++(\x/4,0) to [resistor,l_={$R_{\text{بوجھ}}$}]++(0,\y) to [short,-o]++(-\x/4,0);
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
...
\draw(\x,0) to [resistor,*-*,l_={$\SI{8}{\kilo\ohm}$}]++(0,\y);
\draw(0,\y) to [short,*-]++(0,\y) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(2*\x,0) to [short,-*]++(0,-\y);
\draw(2*\x+\x/4,\y/2)node{$\begin{aligned}&+ \\ &V_{\text{کھلا}} \\ &-  \end{aligned}$};
%loop currents
\draw[stealth-]([shift={(-150:\x/5.5)}]\x/2,\y/2) arc (-150:150:\x/5.5);
\draw(\x/2,\y/2)node{$i_1$};
\draw[stealth-]([shift={(-150:\x/5.5)}]\x,\y+\y/2) arc (-150:150:\x/5.5);
\draw(\x,\y+\y/2)node{$i_2$};
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}%
\begin{subfigure}{0.5\textwidth}
...
\begin{tikzpicture}
\draw(0,0) to [short,-o]++(2*\x+\x/4,0);
\draw(0,0) to [short]++(0,\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{6}{\kilo\ohm}$}]++(\x,0) to [short,-o]++(\x/4,0);
\draw(\x,0) to [resistor,*-*,l_={$\SI{8}{\kilo\ohm}$}]++(0,\y);
\draw[stealth-](2*\x+\x/4,\y/2)--++(\x/4,0)--++(0,-\y/8)node[below]{$R_{\text{تھونن}}$};
\end{tikzpicture}
\caption*{(پ)}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l={$\SI{23.2}{\volt}$}]++(0,\y) to [resistor,l={$\SI{7.6}{\kilo\ohm}$}]++(\x,0) to [short,-o]++(\x/4,0) to [short,o-]++(\x/4,0) to [resistor,i={$i$},l={$R_{\text{بوجھ}}$}]++(0,-\y) to [short,-o]++(-\x/4,0) to [short,o-]++(-\x-\x/4,0);
\end{tikzpicture}
\caption*{(ت)}%
\end{subfigure}
\caption{مثال \حوالہ{مثال_مسئلہ_طاقت_منتقل_الف} کا دور۔}
\label{شکل_مسئلہ_طاقت_منتقلی_الف}
\end{figure}

حل:بوجھ کے علاوہ بقایا دور کا تھونن مساوی حاصل کرتے ہیں۔تھونن دباؤ کی خاطر بوجھ کو ہٹاتے ہوئے شکل \حوالہ{شکل_مسئلہ_طاقت_منتقلی_الف}-ب سے \عددی{V_{\text{کھلا}}} حاصل کرتے ہیں۔شکل-ب سے
\begin{align*}
...
\end{align*}
حاصل ہوتا ہے۔یوں تھونن دباؤ درج ذیل حاصل ہوتا ہے۔
\begin{align*}
V_{\text{کھلا}}=8000i_1+6000i_2 =\SI{23.2}{\volt}
\end{align*}
تھونن مزاحمت حاصل کرنے کی خاطر منبع دباؤ کو قصر دور اور منبع رو کو کھلا دور کرتے ہوئے شکل-پ حاصل کیا گیا جہاں سے
\begin{align*}
...
\end{align*}
ہو۔تھونن مزاحمت اور تھونن دباؤ  کو استعمال کرتے ہوئے تھونن مساوی دور حاصل ہوتا ہے جس کے ساتھ بوجھ جوڑ کر شکل-ت حاصل ہوتا ہے۔بوجھ کے مزاحمت کو تھونن مزاحمت کے برابر لیتے ہوئے
\begin{align*}
...
آئیں بوجھ کی مزاحمت کم اور زیادہ کرتے ہوئے منتقل طاقت حاصل کریں۔بوجھ کی مزاحمت دس فی صد کم کرنے سے 
\begin{align*}
...
آپ دیکھ سکتے ہیں کہ بوجھ کی مزاحمت کو \عددی{R_{\text{تھونن}}} سے کم یا زیادہ کرنے سے بوجھ کو منتقل طاقت  کم ہو جاتا  ہے۔
\انتہا{مثال}
%==================
\ابتدا{مثال}\شناخت{مثال_مسئلہ_طاقت_منتقل_ب}
شکل \حوالہ{شکل_مسئلہ_طاقت_منتقلی_ب} میں مزاحمتی بوجھ کی وہ قیمت دریافت کریں جس پر بوجھ کو زیادہ سے زیادہ طاقت منتقل ہو گی۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
\draw(0,0) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [american controlled voltage source,l={$3000I_x$}]++(\x,0) to [resistor,-o,l={$\SI{4}{\kilo\ohm}$}]++(\x,0);
\draw(\x,0) to [resistor,*-*,l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\draw(2*\x,0) to [resistor,*-*,l={$\SI{4}{\kilo\ohm}$}]++(0,\y);
\draw(3*\x,0) to [resistor,*-*,i<_={$I_x$},l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\draw(4*\x,0) to [resistor,o-o,l_={$R_{\text{بوجھ}}$}]++(0,\y);
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}
\begin{subfigure}{1\textwidth}
...
\begin{tikzpicture}
\draw(0,0) to [short,-o]++(3*\x+\x/2,0);
\draw(0,0) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [american controlled voltage source,l={$3000I_x$}]++(\x,0) to [short,-o]++(\x/2,0);
\draw(\x,0) to [resistor,*-*,l={$\SI{2}{\kilo\ohm}$}]++(0,\y)node[above]{$V_1$};
\draw(2*\x,0) to [resistor,*-*,l={$\SI{4}{\kilo\ohm}$}]++(0,\y)node[above]{$V_2$};
\draw(3*\x,0) to [resistor,*-*,i<_={$I_x$},l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\draw(\x,0) node[ground]{};
\draw(3*\x+\x/2,\y/2)node{$\begin{aligned} &+ \\ &V_{\text{کھلا}} \\ &- \end{aligned}$};
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}
\begin{subfigure}{1\textwidth}
...
\draw(3*\x,0) to [resistor,*-*,i<_={$I_x$},l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\draw(\x,0) node[ground]{};
\draw(3*\x+\x/2,0) to [short,o-o,i<_={$I_{\text{قصر}}$}] ++(0,\y);
\end{tikzpicture}
\caption*{(پ)}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
...
\draw(0,0) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(0,\y) to [short]++(\x,0) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [short,i={$I_{\text{قصر}}$}]++(0,-\y);
\draw(\x,0) to [resistor,*-*,l={$\SI{2}{\kilo\ohm}$}]++(0,\y);
\end{tikzpicture}
\caption*{(ت)}
\end{subfigure}%
...
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l={$\frac{4}{3}\,\si{\volt}$}]++(0,\y) to [resistor,l={$\frac{4}{3}\,\si{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(\x,0) to [resistor,l_={$R_{\text{بوجھ}}$}] ++(0,-\y) to [short]++(-2*\x,0);
\end{tikzpicture}
\caption*{(ٹ)}
\end{subfigure}%
\caption{مثال \حوالہ{مثال_مسئلہ_طاقت_منتقل_ب} کا دور۔}
\label{شکل_مسئلہ_طاقت_منتقلی_ب}
\end{figure}

حل:اس دور پر غور کرنے سے ظاہر ہوتا ہے کہ اگر \عددی{\SI{1}{\kilo\ohm}} کے بالکل دائیں سے دور کو دو ٹکڑوں میں تقسیم کیا جائے تب قصر دور رو نہایت آسانی سے حاصل ہوتی ہے۔ایسا ہی کرتے ہوئے شکل-ب حاصل ہوتا ہے جس سے \عددی{V_{\text{کھلا}}} حاصل کرتے ہیں۔نچلی جوڑ کو زمین تصور کرتے ہوئے بالائی سادہ جوڑ اور مخلوط جوڑ پر کرخوف مساوات رو لکھتے ہیں۔
\begin{align*}
...
قصر دور رو حاصل کرنے کی خاطر شکل-ب کے برقی سروں کو آپس میں قصر دور کرتے ہیں۔ایسا کرنے سے شکل-پ حاصل ہوتا ہے۔اس شکل میں دائیں جانب \عددی{\SI{2}{\kilo\ohm}} کے متوازی قصر پایا جاتا ہے لہٰذا اس مزاحمت میں صفر رو پائی جائے گی  یعنی\عددی{I_x=0} ہو گا۔اس طرح تابع منبع دباؤ صفر وولٹ دباؤ پیدا کرے گا لہٰذا اسے  بھی قصر دور  تصور کیا جا سکتا ہے۔یوں \عددی{\SI{4}{\kilo\ohm}} کے بھی متوازی قصر پایا جائے گا لہٰذا اس میں بھی صفر رو پائی جائے گی۔ان تمام حقائق کو مد نظر رکھتے ہوئے شکل حاصل ہوتا ہے۔تقسیم رو کے کلیے کو استعمال کرتے ہوئے شکل-ت سے
\begin{align*}
...
\end{align*}
تھونن مزاحمت اور دباؤ سے تھونن مساوی دور کے ساتھ بقایا پرزے جوڑنے سے شکل-ٹ حاصل ہوتا ہے۔ یاد رہے کہ زیادہ سے زیادہ طاقت کی منتقلی کے لیے ضروری ہے کہ بوجھ کی مزاحمت بقایا تمام دور کے تھونن مزاحمت کے برابر ہو۔شکل \حوالہ{شکل_مسئلہ_طاقت_منتقلی_ب}-ٹ کو دیکھتے ہوئے یوں زیادہ سے زیادہ طاقت کی منتقلی کے لیے درکار مزاحمت
\begin{align*}
...
\انتہا{مثال}
%===============
\ابتدا{مشق}\شناخت{مشق_مسئلہ_منتقلی_طاقت_الف}
شکل \حوالہ{شکل_مسئلہ_مشق_منتقلی_طاقت_الف} میں زیادہ سے زیادہ طاقت کی منتقلی کے لیے درکار مزاحمت بوجھ دریافت کریں۔

\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [short,-o]++(2*\x,0);
\draw(0,0) to [american voltage source,l={$\SI{4}{\volt}$}]++(0,\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,-o,l={$\SI{1}{\kilo\ohm}$}]++(\x,0) to [resistor,-o,l={$R_{\text{بوجھ}}$}]++(0,-\y);
\draw(\x,0) to [resistor,*-*,l={$\SI{8}{\kilo\ohm}$}]++(0,\y);
\end{tikzpicture}
\caption{مشق \حوالہ{مشق_مسئلہ_منتقلی_طاقت_الف} کا دور۔}
\label{شکل_مسئلہ_مشق_منتقلی_طاقت_الف}
\end{figure}

جواب:\عددی{R_{\text{بوجھ}}=\SI{2.6}{\kilo\ohm}}
\انتہا{مشق}
%===================

\ابتدا{مشق}\شناخت{مشق_مسئلہ_منتقلی_طاقت_ب}
شکل \حوالہ{شکل_مسئلہ_مشق_منتقلی_طاقت_ب} میں زیادہ سے زیادہ طاقت کی منتقلی کے لیے درکار مزاحمت بوجھ دریافت کریں۔زیادہ سے زیادہ منتقل ہونے والی طاقت کی قیمت بھی حاصل کریں۔

\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [short]++(\x,0) to [resistor,l={$\SI{1}{\kilo\ohm}$}]++(\x,0) to [short]++(\x,0);
\draw(0,0) to [american voltage source,l={$\SI{6}{\volt}$}]++(0,\y) to [resistor,l={$\SI{1}{\kilo\ohm}$}]++(\x,0) to [resistor,-o,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{4}{\kilo\ohm}$}] ++(\x,0) to [resistor,l={$R_{\text{بوجھ}}$}]++(0,-\y);
\draw(\x,0) to [american current source,*-*,l={$\SI{4}{\milli\ampere}$}]++(0,\y);
\draw(2*\x,0) to [resistor,*-*,l_={$\SI{8}{\kilo\ohm}$}]++(0,\y);
\end{tikzpicture}
\caption{مشق \حوالہ{مشق_مسئلہ_منتقلی_طاقت_ب} کا دور۔}
\label{شکل_مسئلہ_مشق_منتقلی_طاقت_ب}
\end{figure}
جوابات:\عددی{\tfrac{20}{3}\,\si{\kilo\ohm}} ، \عددی{\tfrac{5}{3}\,\si{\milli\watt}}
\انتہا{مشق}
%===================

\ابتدا{مشق}\شناخت{مشق_مسئلہ_منتقلی_طاقت_پ}
شکل \حوالہ{شکل_مسئلہ_مشق_منتقلی_طاقت_پ} میں زیادہ سے زیادہ طاقت کی منتقلی کے لیے درکار مزاحمت بوجھ دریافت کریں۔

\begin{figure}
\centering
\begin{tikzpicture}
\draw(0,0) to [short]++(4*\x,0);
\draw(0,0) to [american current source,l={$\SI{2}{\milli\ampere}$}]++(0,2*\y) to [short]++(4*\x,0) to  [resistor,l={$R_{\text{بوجھ}}$}]++(0,-2*\y);
\draw(\x,0) to [resistor,i<={$I_x$},*-*,l={$\SI{2}{\kilo\ohm}$}]++(0,2*\y);
\draw(2*\x,0) to [american controlled voltage source,*-,l={$2000I_x$}]++(0,\y) to [resistor,-*,l={$\SI{1}{\kilo\ohm}$}]++(0,\y);
\draw(3*\x,0) to [resistor,*-*,l={$\SI{6}{\kilo\ohm}$}]++(0,2*\y);
\end{tikzpicture}
\caption{مشق \حوالہ{مشق_مسئلہ_منتقلی_طاقت_پ} کا دور۔}
\label{شکل_مسئلہ_مشق_منتقلی_طاقت_پ}
\end{figure}

جواب:\عددی{R_{\text{بوجھ}}=\SI{1.5}{\kilo\ohm}}
\انتہا{مشق}
%===================
...
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_میل_الف}
شکل \حوالہ{شکل_سوال_مسئلے_میل_الف} میں مسئلہ خطی میل کے استعمال سے \عددی{I_0} دریافت کریں۔
\begin{figure}
\centering
\begin{tikzpicture}[american voltages]
...
\end{tikzpicture}
\caption{سوال \حوالہ{سوال_مسئلے_میل_الف} کا دور۔}
\label{شکل_سوال_مسئلے_میل_الف}
\end{figure}

جواب:\عددی{I_0=\tfrac{8}{5}\,\si{\milli\ampere}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_میل_ب}
شکل \حوالہ{شکل_سوال_مسئلے_میل_ب} میں مسئلہ خطی میل کے استعمال سے \عددی{I_0} دریافت کریں۔
\begin{figure}
\centering
\begin{tikzpicture}[american voltages]
...
\end{tikzpicture}
\caption{سوال \حوالہ{سوال_مسئلے_میل_ب} کا دور۔}
\label{شکل_سوال_مسئلے_میل_ب}
\end{figure}

جواب:\عددی{I_0=\SI{1}{\milli\ampere}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_میل_پ}
شکل \حوالہ{شکل_سوال_مسئلے_میل_پ} میں مسئلہ خطی میل کے استعمال سے \عددی{V_0} دریافت کریں۔
\begin{figure}
\centering
\begin{tikzpicture}[american voltages]
...
\end{tikzpicture}
\caption{سوال \حوالہ{سوال_مسئلے_میل_پ} کا دور۔}
\label{شکل_سوال_مسئلے_میل_پ}
\end{figure}

جواب:\عددی{V_0=\SI{13.5}{\volt}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_میل_ت}
شکل \حوالہ{شکل_سوال_مسئلے_میل_ت}-الف میں مسئلہ خطی میل کے استعمال سے \عددی{V_0} دریافت کریں۔
\begin{figure}
\centering
\begin{subfigure}{0.5\textwidth}
\centering
...
\caption*{(ب)}
\end{subfigure}%
\caption{سوال \حوالہ{سوال_مسئلے_میل_ت} اور سوال \حوالہ{سوال_مسئلے_میل_ٹ} کے ادوار۔}
\label{شکل_سوال_مسئلے_میل_ت}
\end{figure}

جواب:\عددی{V_0=\SI{9.6}{\volt}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_میل_ٹ}
شکل \حوالہ{شکل_سوال_مسئلے_میل_ت}-ب میں مسئلہ خطی میل کے استعمال سے \عددی{V_0} دریافت کریں۔
...
شکل \حوالہ{شکل_سوال_مسئلے_تھونن_ٹ}-الف میں \عددی{AB} سروں پر \عددی{\SI{2}{\kilo\ohm}} نصب کرنے سے مزاحمت میں \عددی{\tfrac{5}{2}\,\si{\milli\ampere}} پیدا ہوتی ہے  جبکہ ان سروں پر  \عددی{\SI{6}{\kilo\ohm}} نصب کرنے سے مزاحمت میں \عددی{\tfrac{5}{4}\,\si{\milli\ampere}} پیدا ہوتی ہے۔ دور کے متغیرات \عددی{V_{\text{تھونن}}} اور \عددی{R_{\text{تھونن}}} دریافت کریں۔ 
...
شکل \حوالہ{شکل_سوال_مسئلے_تھونن_ٹ}-ب میں \عددی{AB} سروں پر \عددی{\SI{6}{\kilo\ohm}} نصب کرنے سے  \عددی{V_{AB}=\SI{6}{\volt}} حاصل ہوتا ہے جبکہ  \عددی{\SI{3}{\kilo\ohm}} نصب کرنے سے  \عددی{V_{AB}=\SI{4}{\volt}} حاصل ہوتا ہے۔خطی دور کے تھونن متغیرات \عددی{V_{\text{تھونن}}} اور \عددی{R_{\text{تھونن}}} دریافت کریں۔ 
...
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_بلند_تر_طاقت_پ}
شکل \حوالہ{شکل_سوال_مسئلے_بلند_تر_طاقت_پ}-الف میں بوجھ \عددی{R_L} کی وہ قیمت دریافت کریں جس پر اس کو زیادہ سے زیادہ طاقت منتقل ہو گا۔اس طاقت کا تخمینہ لگائیں۔\عددی{\tfrac{R_L}{2}} اور \عددی{2R_L} بوجھ کی صورت میں بوجھ کو منتقل طاقت دریافت کریں۔
\begin{figure}
\centering
\begin{subfigure}{0.6\textwidth}
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l_={$\SI{8}{\volt}$}]++(0,2*\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$R_L$}]++(\x,0) to [resistor,l={$\SI{4}{\kilo\ohm}$}]++(\x,0);
\draw(\x,0) to [american current source,*-,l={$\SI{4}{\milli\ampere}$}]++(0,\y) to [resistor,-*,l={$\SI{6}{\kilo\ohm}$}]++(0,\y);
\draw(2*\x,0) to [resistor,*-*,l={$\SI{8}{\kilo\ohm}$}]++(0,2*\y);
\draw(3*\x,0) to [american voltage source,l={$\SI{12}{\volt}$}]++(0,2*\y);
\draw(0,0) to [short]++(3*\x,0);
\end{tikzpicture}%
\caption*{(الف)}
\end{subfigure}%
...
\centering
\begin{tikzpicture}
\draw(0,0) to [american controlled voltage source,l={$4I_x$}]++(0,\y) to [resistor,l={$\SI{10}{\ohm}$}]++(0,\y);
\draw(3/4*\x,0) to [resistor,*-,l={$R_L$}]++(0,\y) to [resistor,-*,l={$\SI{5}{\ohm}$}]++(0,\y);
\draw(2*3/4*\x,0) to [american voltage source,l={$\SI{24}{\volt}$}]++(0,\y) to [resistor,l={$\SI{10}{\ohm}$},i<={$I_x$}]++(0,\y);
\draw(0,0) to [short]++(2*3/4*\x,0);
\draw(0,2*\y) to [short]++(2*3/4*\x,0);
\end{tikzpicture}%
\caption*{(ب)}
\end{subfigure}%
\caption{سوال \حوالہ{سوال_مسئلے_بلند_تر_طاقت_پ} اور سوال \حوالہ{سوال_مسئلے_بلند_تر_طاقت_پپ} کے ادوار۔}
\label{شکل_سوال_مسئلے_بلند_تر_طاقت_پ}
\end{figure}

جوابات:\عددی{R_L=\tfrac{14}{3}\,\si{\kilo\ohm}}، \عددی{p=\tfrac{24}{7}\,\si{\milli\watt}}، \عددی{\tfrac{64}{21}\,\si{\milli\watt}}، 
\عددی{\tfrac{64}{21}\,\si{\milli\watt}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_بلند_تر_طاقت_پپ}
شکل \حوالہ{شکل_سوال_مسئلے_بلند_تر_طاقت_پ}-ب میں بوجھ \عددی{R_L} کی وہ قیمت دریافت کریں جس پر اس کو زیادہ سے زیادہ طاقت منتقل ہو گا۔اس طاقت کا تخمینہ لگائیں۔

جوابات:\عددی{R_L=\tfrac{45}{4}\,\si{\ohm}}، \عددی{p=\tfrac{9}{5}\,\si{\watt}}
\انتہا{سوال}
%=======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_بلند_تر_طاقت_ت}
شکل \حوالہ{شکل_سوال_مسئلے_بلند_تر_طاقت_ت}-الف میں بوجھ \عددی{R_L} کی وہ قیمت دریافت کریں جس پر اس کو زیادہ سے زیادہ طاقت منتقل ہو گا۔اس طاقت کا تخمینہ لگائیں۔
\begin{figure}
\centering
\begin{subfigure}{0.5\textwidth}
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l={$\SI{4}{\volt}$}]++(0,\y) to [short]++(0,\y) to [resistor,l={$\SI{2}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{6}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$R_L$}]++(0,-\y);
\draw(\x,0)to [resistor,*-*,l={$\SI{6}{\kilo\ohm}$}]++(0,\y) to [american voltage source,-*,l={$\SI{6}{\volt}$}]++(0,\y);
\draw(2*\x,0) to [american voltage source,-*,l_={$\SI{8}{\volt}$}]++(0,\y);
\draw(0,\y) to [resistor,*-*,l={$\SI{6}{\kilo\ohm}$}]++(\x,0) to [resistor,l={$\SI{6}{\kilo\ohm}$}]++(\x,0);
\draw(0,0) to [short]++(2*\x,0);
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}%
...
\centering
\begin{tikzpicture}[american voltages]
\draw(0,0) to [american voltage source,l={$\SI{8}{\volt}$}]++(0,\y) to [resistor,l={$\SI{2}{\ohm}$}]++(0,\y);
\draw(\x,0) to [american controlled current source,*-,l={$3I_x$}]++(0,\y) to [american controlled voltage source,-*,l={$2V_x$},i={$I_x$}]++(0,\y);
\draw(2*\x,0) to [resistor,l={$\SI{8}{\ohm}$}]++(0,\y) to [american current source,l_={$\SI{2}{\ampere}$}]++(0,\y);
\draw(0,0) to [short]++(2*\x,0);
\draw(0,\y) to [resistor,*-*,l={$\SI{4}{\ohm}$},v_>={$V_x$}]++(\x,0) to [resistor,-*,l={$R_L$}]++(\x,0);
\draw(0,2*\y) to [resistor,l={$\SI{4}{\ohm}$}]++(\x,0) to [resistor,l={$\SI{6}{\ohm}$}]++(\x,0);
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}%
\caption{سوال \حوالہ{سوال_مسئلے_بلند_تر_طاقت_ت} اور سوال \حوالہ{سوال_مسئلے_بلند_تر_طاقت_ٹ} کے ادوار۔}
\label{شکل_سوال_مسئلے_بلند_تر_طاقت_ت}
\end{figure}

جوابات:\عددی{R_L=\SI{7}{\kilo\ohm}}، \عددی{p=\tfrac{1}{28}\,\si{\milli\watt}}
\انتہا{سوال}
%======================
\ابتدا{سوال}\شناخت{سوال_مسئلے_بلند_تر_طاقت_ٹ}
شکل \حوالہ{شکل_سوال_مسئلے_بلند_تر_طاقت_ت}-ب میں بوجھ \عددی{R_L} کی وہ قیمت دریافت کریں جس پر اس کو زیادہ سے زیادہ طاقت منتقل ہو گا۔اس طاقت کا تخمینہ لگائیں۔

جوابات:\عددی{R_L=\tfrac{20}{3}\,\si{\ohm}}، \عددی{p=\tfrac{484}{15}\,\si{\watt}}
\انتہا{سوال}
%======================
