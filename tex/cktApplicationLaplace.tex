...
\centering
\begin{tikzpicture}[american voltages]
\draw(0,0) to [american voltage source,l={$10\,u(t)\,\si{\volt}$}]++(0,2*\yy) to [resistor,l={$\SI{2}{\ohm}$}]++(\xx,0) to [resistor,l={$\SI{2}{\ohm}$}]++(\xx,0) to [inductor,l={$\SI{1}{\henry}$},v={$v_L(t)$}]++(0,-2*\yy) to [short](0,0);
\draw(\xx,0) to[capacitor,*-,l={$\SI{0.5}{\farad}$},v_>={$v_C(t)$}]++(0,\yy) to  [resistor,-*,l={$\SI{1}{\ohm}$}]++(0,\yy);
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}%
\begin{subfigure}{0.5\textwidth}
\centering
\begin{tikzpicture}[american voltages]
\draw(0,0) to [american voltage source,l_={$6\,u(t)\,\si{\volt}$}]++(0,2*\yy) to [resistor,l={$\SI{2}{\ohm}$}]++(\xx,0) to [resistor,l={$\SI{2}{\ohm}$}]++(\xx,0) to [capacitor,l={$\SI{1}{\farad}$},v={$v_0(t)$}]++(0,-2*\yy) to [short](0,0);
\draw(\xx,0) to [resistor,*-,l={$\SI{2}{\ohm}$}]++(0,\yy) to [inductor,-*,l={$\SI{2}{\henry}$}]++(0,\yy);
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}%
...
شکل \حوالہ{شکل_سوال_لاپلاس_مسئلہ_الف} میں مسئلہ نفاذ سے  \عددی{v_0(t)} حاصل کریں۔
\begin{figure}
\centering
\begin{tikzpicture}[american voltages]
...
شکل \حوالہ{شکل_سوال_لاپلاس_مسئلہ_ب} میں مسئلہ نفاذ سے  \عددی{v_0(t)} حاصل کریں۔
\begin{figure}
\centering
\begin{tikzpicture}[american voltages]
