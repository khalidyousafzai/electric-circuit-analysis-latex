...
%======================
\ابتدا{مثال}
\اصطلاح{برقناطیسی امواج}\فرہنگ{برقناطیسی موج}\حاشیہب{electromagnetic waves}\فرہنگ{electromagnetic waves} کو \اصطلاح{اینٹینا}\فرہنگ{اینٹینا}\حاشیہب{antenna}\فرہنگ{antenna} کے ذریعہ خلاء سے حاصل کرتے ہوئے \اصطلاح{ہمسر ایمپلیفائر}\فرہنگ{ہمسر ایمپلیفائر}\حاشیہب{tuned amplifier}\فرہنگ{tuned amplifier} تک پہنچایا جاتا ہے۔ہمسر ایمپلیفائر مخصوص  عرض پٹی کے تعدد کے اشارات کا حیطہ بڑھاتے ہوئے بقایا تعدد کے اشارات کو گھٹاتا ہے۔تعددی طور پر دو قریبی اشارات کو علیحدہ کرنے کے لئے ضروری ہے کہ ہمسر دور کی عرض پٹی اتنی تنگ ہو کہ اس میں سے صرف ایک اشارہ گزر سکے۔ بعض اوقات ایک عدد \عددی{RLC} دور سے اشارات کو علیحدہ کرنا ممکن نہیں ہوتا۔ایسی صورت میں متعدد ہمسر ایمپلیفائر کو زنجیری جوڑا جاتا ہے جہاں پہلے ایمپلیفائر کا خارجی اشارہ دوسرے ایمپلیفائر کو بطور داخلی اشارہ مہیا کیا جاتا ہے۔آئیں دیکھیں کہ زنجیری ایمپلیفائر سے کیسے عرض پٹی مزید تنگ کی جاتی ہے۔

شکل \حوالہ{شکل_تعددی_تنگ_عرض_پٹی}-الف میں زنجیری ایمپلیفائر دکھایا گیا ہے۔داخلی اشارہ \عددی{\hat{V}_1} پہلے ایمپلیفائر کو مہیا کیا گیا ہے۔پہلے ایمپلیفائر کا خارجی اشارہ \عددی{\hat{V}_2} ہے جو دوسرے ایمپلیفائر کو بطور داخلی اشارہ مہیا کیا گیا ہے۔اسی طرح دوسرے ایمپلیفائر کا خارجی اشارہ \عددی{\hat{V}_3} تیسرے ایمپلیفائر کو مہیا کیا گیا ہے۔ شکل-ب میں ایک عدد  ہمسر ایمپلیفائر کا مساوی دور دکھایا گیا ہے۔
\begin{figure}
\centering
\begin{subfigure}{1\textwidth}
\centering
...
\draw(ka)++(-\kl,0) to [short,-o]++(-\x/4,0)node[above]{$\hat{V}_3$};
\draw(ka)++(0,0) to [short,-o]++(\x/4,0)node[above]{$\hat{V}_4$};
\draw(ka)++(-\kl/2,\kh)node[above]{\RL{تیسرا}};
%
...
\draw(ka)++(-\kl/2,\kh)node[above]{\RL{دوسرا}};
%
\draw(-\x-2*\kl,0)coordinate(ka)--++(150:\x/2)--++(0,-\kh)--++(30:\x/2);
\draw(ka)++(-\kl,0) to [short,-o]++(-\x/4,0)node[above]{$\hat{V}_1$};
\draw(ka)++(0,0) to [short,-o]++(\x/4,0);
\draw(ka)++(-\kl/2,\kh)node[above]{\RL{پہلا}};
\end{tikzpicture}
\caption*{(الف)}
\end{subfigure}
\begin{subfigure}{1\textwidth}
\centering
\begin{tikzpicture}
\draw(0,0) to [american controlled current source,l={$A_g \hat{V}_1$}]++(0,-\y)to [short,-o]++(4*\x,0);
...
\draw(-\x,-\y/2)node{$\begin{aligned} &+ \\ &\hat{V}_1 \\ &- \end{aligned}$};
\draw(4*\x,-\y/2)node{$\begin{aligned} &+ \\ &\hat{V}_2 \\ &- \end{aligned}$};
\end{tikzpicture}
\caption*{(ب)}
\end{subfigure}
...
\end{align}
لکھا جا سکتا ہے۔پہلے ایمپلیفائر کا خارجی اشارہ \عددی{\hat{V}_2} ہے جسے دوسرے ایمپلیفائر کو فراہم کیا جاتا ہے لہٰذا مساوات \حوالہ{مساوات_تعددی_متوازی_تبادلی_تفاعل_داخلی_رو} کو دوبارہ استعمال کرتے ہوئے دوسرے ایمپلیفائر کے لئے درج ذیل لکھا جا سکتا ہے
\begin{align*}
\hat{V}_3=\frac{-A_g \hat{V}_2}{G+j(\omega C-\frac{1}{\omega L})}
\end{align*}
جس میں مساوات \حوالہ{مساوات_تعددی_پہلا_ایمپلیفائر} سے \عددی{\hat{V}_2} پر کرتے ہوئے درج ذیل ملتا ہے۔
