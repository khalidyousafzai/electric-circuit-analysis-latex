...
&=\frac{\hat{V}_m}{R}+j\omega C \hat{V}_m+\frac{\hat{V}_m}{j\omega L}\\
&=\hat{V}_m\left[G+j\left(\omega C-\frac{1}{\omega L}\right)\right]
\end{align*}
جہاں آخری قدم پر \عددی{\tfrac{1}{R}=G} لکھا گیا ہے۔گمک تعدد پر \عددی{\omega C=\tfrac{1}{\omega L}} ہوتا ہے لہٰذا رو درج ذیل ہوتی ہے۔
\begin{align*}
\hat{I}_m=G\hat{V}_m
\end{align*}
دور کی داخلی فراوانی \عددی{\bY(j\omega)} درج ذیل ہے
\begin{align}
...
\centering
\begin{tikzpicture}
\draw(0,0) to [american voltage source,l={$\hat{V}_m$}]++(0,\y) to [short,i={$\hat{I}_m$}]++(\x,0) to [short]++(2*\x,0) to [inductor,l={$L$},i={$\hat{I}_L$}]++(0,-\y) to [short] (0,0);
